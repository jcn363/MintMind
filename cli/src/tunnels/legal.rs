...
lazy_static! {
	static ref LICENSE_TEXT: Option<Vec<String>> =
		option_env!("MINTMIND_CLI_SERVER_LICENSE").and_then(|s| serde_json::from_str(s).unwrap());
}

const LICENSE_PROMPT: Option<&'static str> = option_env!("MINTMIND_CLI_REMOTE_LICENSE_PROMPT");
