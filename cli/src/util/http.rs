...
impl SimpleHttp for ReqwestSimpleHttp {
	async fn make_request(
		...
		Ok(SimpleResponse {
			status_code: hyper::StatusCode::from_u16(res.status().as_u16()).unwrap_or(hyper::StatusCode::INTERNAL_SERVER_ERROR),
			headers: {
				let mut headers = hyper::HeaderMap::new();
				for (key, value) in res.headers() {
					if let (Ok(k), Ok(v)) = (hyper::header::HeaderName::from_bytes(key.as_ref()), hyper::header::HeaderValue::from_bytes(value.as_bytes())) {
						headers.insert(k, v);
					}
				}
				headers
