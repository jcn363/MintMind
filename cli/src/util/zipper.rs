		...
		let mut outfile = fs::File::create(&outpath).map_err(|e| {
			wrap(
				...
				format!(
					...
					outpath.display(),
					file.enclosed_name().map(|p| p.to_string_lossy().to_string()),
				),
			)
		})?;
...
#[cfg(unix)]
fn apply_permissions<R: std::io::Read + std::io::Seek>(file: &ZipFile<R>, outpath: &Path) -> Result<(), WrappedError> {
...
#[cfg(windows)]
fn apply_permissions<R: std::io::Read + std::io::Seek>(_file: &ZipFile<R>, _outpath: &Path) -> Result<(), WrappedError> {
	Ok(())
}
