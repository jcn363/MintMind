...
fn set_env_vars_from_map_keys(prefix: &str, map: impl IntoIterator<Item = (String, Value)>) {
		...
		let value = match key.as_str() {
			...
			"nameLong" => {
				if let Value::String(s) = &value {
					let idx = s.find(" - ");
					println!(
						"cargo:rustc-env=MINTMIND_CLI_QUALITYLESS_PRODUCT_NAME={}",
	...
	if !win32_app_ids.is_empty() {
		println!(
			"cargo:rustc-env=MINTMIND_CLI_WIN32_APP_IDS={}",
			win32_app_ids.join(",")
		);
	}
}
...
fn apply_build_from_product_json(path: &Path) {
	...
	set_env_vars_from_map_keys("MINTMIND_CLI", json);
}
...
fn apply_build_environment_variables() {
	...
	println!(
		"cargo:rustc-env=MINTMIND_CLI_VERSION={}",
		package_json.version
	);

	match env::var("MINTMIND_CLI_PRODUCT_JSON") {
		Ok(v) => {
