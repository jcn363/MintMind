...
const logger = createModuleLogger('bootstrap-import');
...
export async function initialize(injectPath: string): Promise<void> {
	...
	for (const [name] of Object.entries(packageJSON.dependencies)) {
		try {
			...
			logger.error(`Error processing dependency ${name}`, err);
		}
	}

	logger.info(`Initialized node_modules redirector for: ${injectPath}`);
}
