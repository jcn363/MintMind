...
import { MINTMIND_AUTHORITY, Schemas } from '../../base/common/network.js';
...
/**
 * The main MintMind application. There will only ever be one instance,
...
export class CodeApplication extends Disposable {
	...
	constructor(
		...
		// Session configuration removed for Tauri mode
	...
	private configureSession(): void {
		...
		const isUrlFromWindow = (requestingUrl?: string | undefined) => requestingUrl?.startsWith(`${Schemas.vscodeFileResource}://${MINTMIND_AUTHORITY}`);
	...
	private registerListeners(): void {
		...
		app.on('web-contents-created', (_event, contents) => {
			...
			if (contents?.opener?.url.startsWith(`${Schemas.vscodeFileResource}://${MINTMIND_AUTHORITY}/`)) {
		...
		if (!isTauriMode()) {
			...
			// Tauri mode handlers are implemented as Rust commands in src-tauri/src/ipc_commands.rs:
			// - fetch_shell_env
			// - toggle_dev_tools
			// - open_dev_tools
			// - reload_window
	...
	async startup(): Promise<void> {
		this.logService.debug('Starting MintMind');
		...
		// "com.microsoft.", which breaks native tabs for MintMind when using this
	...
	private async resolveShellEnvironment(args: NativeParsedArgs, env: IProcessEnvironment, notifyOnError: boolean): Promise<typeof process.env> {
		if (isTauriMode()) {
			try {
				return await invoke('fetch_shell_env', { args, env });
			} catch (error) {
