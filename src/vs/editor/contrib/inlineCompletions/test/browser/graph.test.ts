...
describe('DirectedGraph', () => {
	...
	it('from - handles duplicate edges', () => {
		const nodes = ['a', 'b'];
		const getOutgoing = (node: string) => {
			switch (node) {
				case 'a':
	...
	it('removeCycles - multiple disconnected cycles', () => {
		const nodes = ['a', 'b', 'c', 'd'];
		const getOutgoing = (node: string) => {
			switch (node) {
				case 'a':
	...
	it('getOutgoing - non-existent node', () => {
		const graph = DirectedGraph.from(['a'], () => []);
		assert.deepStrictEqual(graph.getOutgoing('b'), []);
	});

	it('with number nodes', () => {
		const nodes = [1, 2, 3];
