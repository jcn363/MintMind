...
describe('SuggestController', function () {
	...
	it('postfix completion reports incorrect position #86984', async function () {
	...
	it('use additionalTextEdits sync when possible', async function () {
	...
	it('resolve additionalTextEdits async when needed', async function () {
	...
	it('resolve additionalTextEdits async when needed (typing)', async function () {
	...
	it('resolve additionalTextEdits async when needed (simple conflict)', async function () {
	...
	it('resolve additionalTextEdits async when needed (conflict)', async function () {
	...
	it('resolve additionalTextEdits async when needed (cancel)', async function () {
	...
	it('Completion edits are applied inconsistently when additionalTextEdits and textEdit start at the same offset #143888', async function () {
	...
	it('Pressing enter on autocomplete should always apply the selected dropdown completion, not a different, hidden one #161883', async function () {
		disposables.add(languageFeaturesService.completionProvider.register({ scheme: 'test-ctrl' }, {
			_debugDisplayName: 'test',
			provideCompletionItems(doc, pos) {
	...
	it('Fast autocomple typing selects the previous autocomplete suggestion, #71795', async function () {
		disposables.add(languageFeaturesService.completionProvider.register({ scheme: 'test-ctrl' }, {
			_debugDisplayName: 'test',
			provideCompletionItems(doc, pos) {
	...
	it.skip('Suggest widget gets orphaned in editor #187779', async function () {
	...
	it('Ranges where additionalTextEdits are applied are not appropriate when characters are typed #177591', async function () {
		disposables.add(languageFeaturesService.completionProvider.register({ scheme: 'test-ctrl' }, {
			_debugDisplayName: 'test',
			provideCompletionItems(doc, pos) {
				return {
	...
	it.skip('[Bug] "No suggestions" persists while typing if the completion helper is set to return an empty list for empty content#3557', async function () {
		let requestCount = 0;
