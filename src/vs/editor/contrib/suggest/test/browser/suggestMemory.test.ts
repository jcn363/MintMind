...
describe('SuggestMemories', function () {
	...
	it('AbstractMemory, select', function () {
	...
	it('[No|Prefix|LRU]Memory honor selection boost', function () {
		const item1 = createSuggestItem('fazz', 0);
		const item2 = createSuggestItem('bazz', 0);
		const item3 = createSuggestItem('bazz', 0);
		const item4 = createSuggestItem('bazz', 0);
	...
	it('`"editor.suggestSelection": "recentlyUsed"` should be a little more sticky #78571', function () {
	...
	it('intellisense is not showing top options first #43429', function () {
