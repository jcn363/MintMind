...
describe('Editor Core - Editor State', () => {
	...
	it('empty editor state should be valid', () => {
		const result = validate({}, {});
		assert.strictEqual(result, true);
	});

	it('different model URIs should be invalid', () => {
		const result = validate(
	...
	it('different model versions should be invalid', () => {
		const result = validate(
			{ model: { version: 1 } },
			{ model: { version: 2 } }
		);
	...
	it('different positions should be invalid', () => {
		const result = validate(
	...
	it('different selections should be invalid', () => {
		const result = validate(
	...
	it('different scroll positions should be invalid', () => {
		const result = validate(
			{ scroll: { left: 1, top: 2 } },
			{ scroll: { left: 3, top: 2 } }
		);
