...
describe('TokensStore', () => {
	...
	it('issue #86303 - color shifting between different tokens', () => {
	...
	it('issue #179268: a complex edit', () => {
		testTokensAdjustment(
			[
	...
	it('issue #91936: Semantic token color highlighting fails on line with selected text', () => {
	...
	it('issue #147944: Language id "vs.editor.nullLanguage" is not configured nor known', () => {
	...
	it('issue #94133: Semantic colors stick around when using (only) range provider', () => {
		const codec = new LanguageIdCodec();
		const store = new SparseTokensStore(codec);
	...
	it('issue #95949: Identifiers are colored in bold when targetting keywords', () => {
	...
	it('BUG: setPartial with startLineNumber > 1 and token removal creates invalid state', () => {
		/**
	...
	it('BUG: setPartial with split that creates empty first piece with invalid line numbers', () => {
		const codec = new LanguageIdCodec();
		const store = new SparseTokensStore(codec);
	...
	it('piece with startLineNumber 0 and endLineNumber -1 after encompassing deletion', () => {
		const codec = new LanguageIdCodec();
		const store = new SparseTokensStore(codec);
