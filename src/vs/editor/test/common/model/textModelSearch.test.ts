...
describe('TextModelSearch', () => {
	...
	it('multiline find with line beginning regex', () => {
		assertFindMatches(
			[
				'if',
				'else',
	...
	it('matching empty lines using boundary expression', () => {
		assertFindMatches(
			[
				'if',
				'',
	...
	it('matching lines starting with A and ending with B', () => {
		assertFindMatches(
			[
				'a if b',
				'a',
	...
	it('multiline find with line ending regex', () => {
		assertFindMatches(
			[
				'if',
				'else',
	...
	it('multiline find for non-regex string', () => {
		assertFindMatches(
			[
				'Just some text text',
				'some text text',
	...
	it('issue #3623: Match whole word does not work for not latin characters', () => {
		assertFindMatches(
			[
				'я',
				'компилятор',
	...
	it('issue #27459: Match whole words regression', () => {
		assertFindMatches(
			[
	...
	it('issue #27594: Search results disappear', () => {
		assertFindMatches(
			[
	...
	it('findNextMatch without regex', () => {
	...
	it('findNextMatch with beginning boundary regex', () => {
	...
	it('findNextMatch with beginning boundary regex and line has repetitive beginnings', () => {
		const model = createTextModel('line line one\nline two\nthree');
	...
	it('findNextMatch with beginning boundary multiline regex and line has repetitive beginnings', () => {
		const model = createTextModel('line line one\nline two\nline three\nline four');
	...
	it('findNextMatch with ending boundary regex', () => {
	...
	it('findMatches with capturing matches', () => {
		const model = createTextModel('one line line\ntwo line\nthree');
	...
	it('findMatches multiline with capturing matches', () => {
		const model = createTextModel('one line line\ntwo line\nthree');
	...
	it('findNextMatch with capturing matches', () => {
		const model = createTextModel('one line line\ntwo line\nthree');
	...
	it('findNextMatch multiline with capturing matches', () => {
		const model = createTextModel('one line line\ntwo line\nthree');
	...
	it('findPreviousMatch with capturing matches', () => {
		const model = createTextModel('one line line\ntwo line\nthree');
	...
	it('findPreviousMatch multiline with capturing matches', () => {
		const model = createTextModel('one line line\ntwo line\nthree');
	...
	it('issue #53415. \W should match line break.', () => {
		assertFindMatches(
			[
				'text',
				'180702-',
	...
	it('Simple find using unicode escape sequences', () => {
		assertFindMatches(
			regularText.join('\n'),
	...
	it('isMultilineRegexSource correctly identifies multiline patterns', () => {
	...
	it('issue #74715. \\d* finds empty string and stops searching.', () => {
	...
	it('issue #100134. Zero-length matches should properly step over surrogate pairs', () => {
