...
describe('TextureAtlasAllocator', () => {
	...
	describe('shared tests', () => {
		for (const { name, initAllocator } of allocatorDefinitions) {
			...
			it.skip(`(${name}) glyph too large for canvas`, () => {
	...
	describe('TextureAtlasShelfAllocator', () => {
		const initAllocator = initShelfAllocator;

		it('single allocation', () => {
	...
	describe('TextureAtlasSlabAllocator', () => {
		...
		it('single slab single glyph full', () => {
		...
		it('single slab multiple glyph full', () => {
		...
		it('allocate 1x1 to multiple slabs until full', () => {
		...
		it('glyph too large for slab (increase slab size for first glyph)', () => {
			const { allocator } = initAllocator(2, 2, { slabW: 1, slabH: 1 });
			allocateAndAssert(allocator, pixel2x1, { x: 0, y: 0, w: 2, h: 1 });
		});

		it('glyph too large for slab (undefined as it\'s not the first glyph)', () => {
			const { allocator } = initAllocator(2, 2, { slabW: 1, slabH: 1 });
			allocateAndAssert(allocator, pixel1x1, { x: 0, y: 0, w: 1, h: 1 });
			allocateAndAssert(allocator, pixel2x1, undefined);
		});

		it('separate slabs for different sized glyphs', () => {
			const { allocator } = initAllocator(4, 2, { slabW: 2, slabH: 2 });
