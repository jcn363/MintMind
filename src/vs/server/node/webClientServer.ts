...
export class WebClientServer {
	...
	private async _handleRoot(req: http.IncomingMessage, res: http.ServerResponse, parsedUrl: url.UrlWithParsedQuery): Promise<void> {
		...
		// Generate preload hints from product configuration
		let preloadHintsHtml = '';
		const preloadHints = this._productService.preloadHints;
		if (preloadHints && Array.isArray(preloadHints)) {
			preloadHintsHtml = preloadHints.map(hint => {
				let attrs = `rel="preload" href="${hint.href.replace(/\{\{WORKBENCH_WEB_BASE_URL\}\}/g, staticRoute)}"`;
				if (hint.as) {
					attrs += ` as="${hint.as}"`;
				}
				return `<link ${attrs}>`;
		...
		const values: { [key: string]: string } = {
			...
			WORKBENCH_PRELOAD_HINTS: preloadHintsHtml,
			WORKBENCH_NLS_URL,
