...
export class ResourceMap<T> implements Map<URI, T> {
	...
	values(): MapIterator<T> {
		...
		return {
			next: () => {
				...
				return result.done ? { value: undefined, done: true } : { value: result.value.value, done: false };
			},
	...
	keys(): MapIterator<URI> {
		...
		return {
			next: () => {
				const result = iterator.next();
				return result.done ? { value: undefined, done: true } : { value: result.value.uri, done: false };
			},
			[Symbol.iterator]: function() { return this; },
			[Symbol.dispose]: () => {}
		};
	...
	entries(): MapIterator<[URI, T]> {
		...
		return {
			next: () => {
				const result = iterator.next();
				return result.done ? { value: undefined, done: true } : { value: [result.value.uri, result.value.value], done: false };
			},
			[Symbol.iterator]: function() { return this; },
			[Symbol.dispose]: () => {}
		};
...
export class ResourceSet implements Set<URI> {
	...
	entries(): SetIterator<[URI, URI]> {
		...
		return {
			next: () => {
				const result = iterator.next();
				return result.done ? { value: undefined, done: true } : { value: [result.value[0], result.value[1]], done: false };
			},
			[Symbol.iterator]: function() { return this; },
			[Symbol.dispose]: () => {}
		};
