...
describe('CancellationTokenPool', function () {

	const store = ensureNoDisposablesAreLeakedInTestSuite();

	it('empty pool token is not cancelled', function () {
	...
	it('pool token cancels when all tokens are cancelled', function () {
		const pool = new CancellationTokenPool();
		store.add(pool);
	...
	it('pool token fires cancellation event when all tokens are cancelled', function () {
		return new Promise<void>(resolve => {
			const pool = new CancellationTokenPool();
			store.add(pool);
	...
	it('adding already cancelled token counts immediately', function () {
		const pool = new CancellationTokenPool();
		store.add(pool);
	...
	it('adding single already cancelled token cancels pool immediately', function () {
		const pool = new CancellationTokenPool();
		store.add(pool);
	...
	it('adding token after pool is done has no effect', function () {
		const pool = new CancellationTokenPool();
		store.add(pool);
	...
	it('single token pool behaviour', function () {
		const pool = new CancellationTokenPool();
		store.add(pool);
	...
	it('pool with only cancelled tokens', function () {
		const pool = new CancellationTokenPool();
		store.add(pool);
