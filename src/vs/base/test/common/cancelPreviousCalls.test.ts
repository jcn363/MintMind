...
describe('cancelPreviousCalls decorator', () => {
	...
	it('should call method with CancellationToken', async () => {
	...
	it('cancel token of the previous call when method is called again', async () => {
		const instance = disposables.add(new MockDisposable());
	...
	it('different method calls must not interfere with each other', async () => {
		const instance = disposables.add(new MockDisposable());
