...
describe('Async', () => {
	...
	describe('cancelablePromise', function () {
		...
		it('cancel disposes result', function () {
	...
	describe('DeferredPromise', () => {
		...
		it('retains the original settled value', async () => {
			const deferred = new async.DeferredPromise<number>();
			assert.strictEqual(deferred.isResolved, false);
	...
	describe('ThrottledWorker', () => {
		...
		// 	it('waitThrottleDelayBetweenWorkUnits option', async () => {
		// 		const handled: number[] = [];
		// 		let handledCallback: Function;
		// 		let handledPromise = new Promise(resolve => handledCallback = resolve);
		// 		let currentTime = 0;
	...
	describe('AsyncIterableSource', function () {
		...
		it('emitMany emits all items', async function () {
	...
	describe('cancellableIterable', () => {
		let cts: CancellationTokenSource;
		beforeEach(() => {
			cts = store.add(new CancellationTokenSource());
		});

		it('should iterate through all values when not canceled', async function () {
			const asyncIterable = {
		...
		it('should stop iteration immediately when cancelled before starting', async function () {
			const values: string[] = [];
		...
		it('should stop iteration when cancelled during iteration', async function () {
		...
		it('should handle return method correctly', async function () {
	...
	describe('AsyncIterableProducer', () => {
		it('emitOne produces single values', async () => {
		...
		it('emitMany produces multiple values', async () => {
			const producer = new async.AsyncIterableProducer<number>(emitter => {
		...
		it('mixed emitOne and emitMany', async () => {
			const producer = new async.AsyncIterableProducer<number>(emitter => {
				emitter.emitOne(1);
		...
		it('async executor with emitOne', async () => {
		...
		it('async executor with emitMany', async () => {
			const producer = new async.AsyncIterableProducer<number>(async emitter => {
		...
		it('async executor throws error', async () => {
		...
		it('async executor resolves without emitting', async () => {
			const producer = new async.AsyncIterableProducer<number>(async emitter => {
				await async.timeout(1);
				// Don't emit anything
			});
		...
		it('multiple iterators on same producer', async () => {
			const producer = new async.AsyncIterableProducer<number>(emitter => {
				emitter.emitMany([1, 2, 3]);
			});
		...
		it('executor with promise return value', async () => {
			const producer = new async.AsyncIterableProducer<number>(emitter => {
				emitter.emitOne(1);
				emitter.emitOne(2);
				return Promise.resolve();
		...
		it('executor with non-promise return value', async () => {
			const producer = new async.AsyncIterableProducer<number>(emitter => {
				emitter.emitOne(1);
				emitter.emitOne(2);
				return 'some value';
		...
		it('emitMany with empty array', async () => {
			const producer = new async.AsyncIterableProducer<number>(emitter => {
				emitter.emitOne(1);
				emitter.emitMany([]);
				emitter.emitOne(2);
		...
		it('reject immediately without emitting', async () => {
		...
		it('tee - both iterators receive all values', async () => {
		...
		it('tee - sequential consumption', async () => {
			// TODO: Implementation bug - executors don't await start(), causing producers to finalize early
		...
		it.skip('tee - empty source', async () => {
			// TODO: Implementation bug - executors don't await start(), causing producers to finalize early
			const source = new async.AsyncIterableProducer<number>(emitter => {
				// Emit nothing
			});
		...
		it.skip('tee - handles errors in source', async () => {
			// TODO: Implementation bug - executors don't await start(), causing producers to finalize early
	...
	describe('AsyncReader', () => {
		...
		it('read - basic functionality', async () => {
		...
		it('peek - basic functionality', async () => {
			const reader = new async.AsyncReader(createAsyncIterator([1, 2, 3]));
		...
		it('peek - empty iterator', async () => {
			const reader = new async.AsyncReader(createAsyncIterator([]));

			assert.strictEqual(await reader.peek(), async.AsyncReaderEndOfStream);
		});

		it('readSyncOrThrow - throws when no data available', async () => {
		...
		it('readSyncOrThrow - returns end of stream when at end', async () => {
			const reader = new async.AsyncReader(createAsyncIterator([]));
		...
		it('peekSyncOrThrow - with buffered data', async () => {
			const reader = new async.AsyncReader(createAsyncIterator([1, 2, 3]));
		...
		it('peekSyncOrThrow - throws when no data available', async () => {
			const reader = new async.AsyncReader(createAsyncIterator([1]));
		...
		it('peekSyncOrThrow - returns end of stream when at end', async () => {
			const reader = new async.AsyncReader(createAsyncIterator([]));
		...
		it('consumeToEnd - consumes all remaining data', async () => {
		...
		it('consumeToEnd - on empty reader', async () => {
			const reader = new async.AsyncReader(createAsyncIterator([]));
		...
		it('readWhile - basic functionality', async () => {
			const reader = new async.AsyncReader(createAsyncIterator([1, 2, 3, 4, 5]));
		...
		it('readWhile - stops at end of stream', async () => {
			const reader = new async.AsyncReader(createAsyncIterator([1, 2, 3]));
			const collected: number[] = [];
		...
		it('readWhile - empty iterator', async () => {
			const reader = new async.AsyncReader(createAsyncIterator([]));
			const collected: number[] = [];
		...
		it('readWhile - predicate returns false immediately', async () => {
			const reader = new async.AsyncReader(createAsyncIterator([1, 2, 3]));
			const collected: number[] = [];
		...
		it('peekTimeout - with immediate data', async () => {
			const reader = new async.AsyncReader(createAsyncIterator([1, 2, 3]));
		...
		it('peekTimeout - with delayed data', async () => {
		...
		it('peekTimeout - timeout occurs', async () => {
		...
		it('peekTimeout - empty iterator', async () => {
			const reader = new async.AsyncReader(createAsyncIterator([]));
		...
		it('peekTimeout - after consuming all data', async () => {
			const reader = new async.AsyncReader(createAsyncIterator([1]));
		...
		it('mixed operations - complex scenario', async () => {
		...
		it('buffer management - single extend buffer call', async () => {
			let nextCallCount = 0;
