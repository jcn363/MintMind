...
describe('IterativePagedModel', () => {
	...
	it('load first page via sentinel access', async () => {
		const pager = createTestPager(10, 3);
		const model = store.add(new IterativePagedModel(pager));
	...
	it('onDidIncrementLength event fires correctly', async () => {
		const pager = createTestPager(10, 3);
		const model = store.add(new IterativePagedModel(pager));
	...
	it('accessing regular items does not trigger loading', async () => {
		const pager = createTestPager(10, 3);
		const model = store.add(new IterativePagedModel(pager));
	...
	it('reaching end of data removes sentinel', async () => {
		const pager = createTestPager(10, 3);
		const model = store.add(new IterativePagedModel(pager));
	...
	it('concurrent access to sentinel only loads once', async () => {
		const pager = createTestPager(10, 3);
		const model = store.add(new IterativePagedModel(pager));
	...
	it('single page pager with no more pages', () => {
	...
	it('accessing item beyond loaded range throws', () => {
		const pager = createTestPager(10, 3);
		const model = store.add(new IterativePagedModel(pager));
	...
	it('resolving item beyond all pages throws', async () => {
		const pager = createTestPager(10, 3);
		const model = store.add(new IterativePagedModel(pager));
	...
	it('cancelled token during initial resolve', async () => {
		const pager = createTestPager(10, 3);
		const model = store.add(new IterativePagedModel(pager));
	...
	it('event fires for each page load', async () => {
	...
	it('sequential page loads work correctly', async () => {
	...
	it('accessing items after loading all pages', async () => {
	...
	it('pager with varying page sizes', async () => {
		let pageNum = 0;
