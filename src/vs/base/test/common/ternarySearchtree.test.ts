...
describe('Ternary Search Tree', () => {
	...
	it('TernarySearchTree - set w/ undefined', function () {
	...
	it('https://github.com/microsoft/vscode/issues/227147', function () {
	...
	it('TernarySearchTree (URI) - lookup, casing', function () {
		...
		const map = new TernarySearchTree<URI, number>(new UriIterator(uri => /^https?$/.it(uri.scheme), () => false));
		map.set(URI.parse('http://foo.bar/user/foo/bar'), 1);
		assert.strictEqual(map.get(URI.parse('http://foo.bar/USER/foo/bar')), 1);
