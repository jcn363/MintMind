...
flakySuite('ID', () => {
	...
	it('getMachineId', async function () {
		const errors = [];
	...
	it('getDevDeviceId', async function () {
		const errors = [];
	...
	it('getMac', async () => {
		...
		assert.ok(/^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$/.it(macAddress), `Expected a MAC address, got: ${macAddress}`);
	});
});
