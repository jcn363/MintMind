...
describe('ObjectTreeModel', function () {
	...
	it('setChildren on collapsed node', () => {
		const list: ITreeNode<number>[] = [];
		const model = new ObjectTreeModel<number>('test');
		const disposable = bindListToModel(list, model);
	...
	it('setChildren on expanded, unrevealed node', () => {
		const list: ITreeNode<number>[] = [];
		const model = new ObjectTreeModel<number>('test');
		const disposable = bindListToModel(list, model);
	...
	it('collapse state is preserved with strict identity', () => {
	...
	it('collapse state can be optionally preserved with strict identity', () => {
		const list: ITreeNode<string>[] = [];
		const model = new ObjectTreeModel<string>('test', { collapseByDefault: true });
