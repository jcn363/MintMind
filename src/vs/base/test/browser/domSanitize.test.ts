...
describe('DomSanitize', () => {
	...
	it('removes unsupported tags by default', () => {
	...
	it('removes unsupported attributes by default', () => {
	...
	it('allows custom tags via config', () => {
		{
	...
	it('allows custom attributes via config', () => {
	...
	it('Attributes in config should be case insensitive', () => {
		const html = '<div Custom-Attr="value">content</div>';
	...
	it('removes unsupported protocols for href by default', () => {
	...
	it('removes unsupported protocols for src by default', () => {
	...
	it('allows safe protocols for href', () => {
	...
	it('removes data images by default', () => {
	...
	it('Removes relative paths for img src by default', () => {
		const html = '<img src="path/img.png">';
		const result = sanitizeHtml(html);
		assert.strictEqual(result.toString(), '<img>');
	});

	it('Can allow relative paths for image', () => {
		const html = '<img src="path/img.png">';
		const result = sanitizeHtml(html, {
	...
	it('Supports dynamic attribute sanitization', () => {
	...
	it('Supports changing attributes in dynamic sanitization', () => {
	...
	it('Attr name should clear previously set dynamic sanitizer', () => {
		const html = '<div title="abc" other="1">text1</div><div title="xyz" other="2">text2</div>';
		const result = sanitizeHtml(html, {
			allowedAttributes: {
				override: [
	...
	describe('replaceWithPlaintext', () => {

		it('replaces unsupported tags with plaintext representation', () => {
			const html = '<div>safe<script>alert(1)</script>content</div>';
			const result = sanitizeHtml(html, {
		...
		it('works with custom allowed tags configuration', () => {
