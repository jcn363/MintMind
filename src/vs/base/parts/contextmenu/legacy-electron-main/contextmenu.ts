...
export async function registerContextMenuListener(): Promise<void> {
	if (isTauriMode()) {
		const { listen } = await import('@tauri-apps/api/event');
		await listen(CONTEXT_MENU_CHANNEL, async (event) => {
			const { contextMenuId, items, onClickChannel, options } = event.payload;
			// TODO: Implement Tauri context menu - currently not implemented
			console.warn('Tauri context menu not yet implemented');
			return;
		});
	} else {
