...
(function () {
	...
	const ipcRenderer = {
		...
		on(channel: string, listener: (event: any, ...args: unknown[]) => void) {
			window.__TAURI__.event.listen(channel, (event: any) => listener(event, ...event.payload));
			return this;
		},
		once(channel: string, listener: (event: any, ...args: unknown[]) => void) {
			window.__TAURI__.event.once(channel, (event: any) => listener(event, ...event.payload));
			return this;
		},
		removeListener(channel: string, listener: (event: any, ...args: unknown[]) => void) {
			// Tauri doesn't have direct removeListener, this needs to be handled differently
			return this;
		}
	};
	...
	const webUtils = {
		getPathForFile(file: File): string {
			// Tauri doesn't have direct equivalent, return the file name
			return file.name;
		}
	};
	...
	/**
	 * If MintMind is not run from a terminal, we should resolve additional
	...
	const globals = {
		...
		ipcMessagePort: {
			...
			acquire(responseChannel: string, nonce: string) {
				if (validateIPC(responseChannel)) {
					const responseListener = (e: any, responseNonce: string) => {
					...
					// handle reply from main
							ipcRenderer.on(responseChannel, responseListener as any);
				}
			}
		},
		...
		process: {
			...
			cwd(): string {
				return process.env['MINTMIND_CWD'] || process.execPath.substr(0, process.execPath.lastIndexOf(process.platform === 'win32' ? '\\' : '/'));
			},
			...
			getProcessMemoryInfo(): Promise<any> {
				// Tauri doesn't have direct equivalent for process memory info
				// Return a basic structure
				return Promise.resolve({ private: 0, shared: 0 });
			},
	...
	try {
		// Use `contextBridge` APIs to expose globals to MintMindnd
