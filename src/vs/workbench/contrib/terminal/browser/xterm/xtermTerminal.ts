...
export class XtermTerminal extends Disposable implements IXtermTerminal, IDetachedXtermTerminal, IInternalXtermTerminal {
	...
	constructor(
		...
		this._xtermAddonLoader.importAddon('clipboard').then(ClipboardAddon => {
			...
			this._clipboardAddon = this._instantiationService.createInstance(ClipboardAddon, {
	...
	*getBufferReverseIterator(): IterableIterator<string> {
	...
	updateConfig(): void {
		...
		// Note: overviewRuler is not a valid option for xterm.js
		this._updateSmoothScrolling();
		if (this._attached) {
	...
	getXtermTheme(theme?: IColorTheme): ITheme {
		...
		return {
			...
			// Note: overviewRulerBorder is not a valid theme property for xterm.js
