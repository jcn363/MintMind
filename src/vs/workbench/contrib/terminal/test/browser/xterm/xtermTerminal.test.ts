...
describe('XtermTerminal', () => {
	...
	it('should use fallback dimensions of 80x30', () => {
		strictEqual(xterm.raw.cols, 80);
		strictEqual(xterm.raw.rows, 30);
	});

	describe('getContentsAsText', () => {
		it('should return all buffer contents when no markers provided', async () => {
			await write('line 1\r\nline 2\r\nline 3\r\nline 4\r\nline 5');
		...
		it('should return contents from start marker to end', async () => {
			await write('line 1\r\n');
		...
		it('should return contents from start to end marker', async () => {
			await write('line 1\r\n');
			const startMarker = xterm.raw.registerMarker(0)!;
			await write('line 2\r\nline 3\r\n');
		...
		it('should return single line when start and end markers are the same', async () => {
			await write('line 1\r\nline 2\r\n');
		...
		it('should return empty string when start marker is beyond end marker', async () => {
			await write('line 1\r\n');
			const endMarker = xterm.raw.registerMarker(0)!;
			await write('line 2\r\nline 3\r\n');
			const startMarker = xterm.raw.registerMarker(0)!;
		...
		it('should handle mixed content with spaces and special characters', async () => {
		...
		it('should throw error when startMarker is disposed (line === -1)', async () => {
			await write('line 1\r\n');
		...
		it('should throw error when endMarker is disposed (line === -1)', async () => {
			await write('line 1\r\n');
			const startMarker = xterm.raw.registerMarker(0)!;
			await write('line 2\r\n');
		...
		it('should handle markers at buffer boundaries', async () => {
			const startMarker = xterm.raw.registerMarker(0)!;
			await write('line 1\r\nline 2\r\nline 3\r\nline 4\r\n');
			const endMarker = xterm.raw.registerMarker(0)!;
			await write('line 5');
		...
		it('should handle terminal escape sequences properly', async () => {
	...
	describe('getBufferReverseIterator', () => {
		it('should get text properly within scrollback limit', async () => {
			const text = 'line 1\r\nline 2\r\nline 3\r\nline 4\r\nline 5';
			await write(text);
		...
		it('should get text properly when exceed scrollback limit', async () => {
	...
	describe('theme', () => {
		it('should apply correct background color based on getBackgroundColor', () => {
		...
		it('should react to and apply theme changes', () => {
			themeService.setTheme(new TestColorTheme({
