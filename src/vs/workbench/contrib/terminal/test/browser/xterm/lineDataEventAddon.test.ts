...
describe('LineDataEventAddon', () => {
	...
	describe('onLineData', () => {
		...
		it('should fire when a non-wrapped line ends with a line feed', async () => {
		...
		it('should not fire soft wrapped lines', async () => {
			await writeP(xterm, 'foo.');
			deepStrictEqual(events, []);
			await writeP(xterm, 'bar.');
			deepStrictEqual(events, []);
		...
		it('should fire when a wrapped line ends with a line feed', async () => {
		...
		it('should not fire on cursor move when the backing process is not on Windows', async () => {
			await writeP(xterm, 'foo.\x1b[H');
			deepStrictEqual(events, []);
		});

		it('should fire on cursor move when the backing process is on Windows', async () => {
