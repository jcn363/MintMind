...
describe('PartialCommandDetectionCapability', () => {
	...
	it('should not add commands when the cursor position is too close to the left side', async () => {
		assertCommands([]);
		xterm.input('\x0d');
		await writeP(xterm, '\r\n');
		assertCommands([]);
	...
	it('should add commands when the cursor position is not too close to the left side', async () => {
		assertCommands([]);
	...
	it('onDidExecuteText should cause onDidCommandFinished to fire', async () => {
		await writeP(xterm, 'cd');
