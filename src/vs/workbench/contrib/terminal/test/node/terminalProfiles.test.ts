...
describe('Workbench - TerminalProfiles', () => {
	...
	describe('detectAvailableProfiles', () => {
		if (isWindows) {
			it('should detect Git Bash and provide login args', async () => {
			...
			it('should allow source to have args', async () => {
			...
			it('configured args should override default source ones', async () => {
				const fsProvider = createFsProvider([
					'C:\\Program Files\\Git\\bin\\bash.exe'
				]);
				const config: ITestTerminalConfig = {
			...
			describe('pwsh source detection/fallback', () => {
				...
				it('should prefer pwsh 7 to Windows PowerShell', async () => {
					const pwshSourcePaths = [
						'C:\\Program Files\\PowerShell\\7\\pwsh.exe',
				...
				it('should prefer pwsh 7 to pwsh 6', async () => {
					const pwshSourcePaths = [
						'C:\\Program Files\\PowerShell\\7\\pwsh.exe',
				...
				it('should fallback to Windows PowerShell', async () => {
					const pwshSourcePaths = [
			...
			it('should detect shells via absolute paths', async () => {
				const fsProvider = createFsProvider([
					'/bin/fakeshell1',
					'/bin/fakeshell3'
				]);
			...
			it('should auto detect shells via /etc/shells', async () => {
				const fsProvider = createFsProvider([
					'/bin/fakeshell1',
					'/bin/fakeshell3'
				], '/bin/fakeshell1\n/bin/fakeshell3');
			...
			it('should validate auto detected shells from /etc/shells exist', async () => {
