...
describe('Files - View Model', function () {
	...
	it('Add and Remove Child, check for hasChild', function () {
		const d = new Date().getTime();
	...
	it('Find with mixed case', function () {
		const d = new Date().getTime();
	...
	it('Validate File Name (For Create)', function () {
		const d = new Date().getTime();
	...
	it('Validate File Name (For Rename)', function () {
		const d = new Date().getTime();
		const s = createStat.call(this, '/path/to/stat', 'sName', true, true, 8096, d);
		const sChild = createStat.call(this, '/path/to/stat/alles.klar', 'alles.klar', true, true, 8096, d);
		s.addChild(sChild);
	...
	it('Validate Multi-Path File Names', function () {
		const d = new Date().getTime();
	...
	it('Merge Local with Disk', function () {
