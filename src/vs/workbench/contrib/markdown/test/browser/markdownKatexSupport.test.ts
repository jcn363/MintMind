...
describe('Markdown Katex Support Test', () => {
	...
	it('Basic inline equation', async () => {
	...
	it('Should support inline equation wrapped in parans', async () => {
		const rendered = await renderMarkdownWithKatex('Hello ($\\frac{1}{2}$) World!');
		assert.ok(rendered.element.innerHTML.includes('katex'));
		await assertSnapshot(rendered.element.innerHTML);
	});

	it('Should support blocks immediately after paragraph', async () => {
	...
	it('Should not render math when dollar sign is preceded by word character', async () => {
		const rendered = await renderMarkdownWithKatex('for ($i = 1; $i -le 20; $i++) { echo "hello world"; Start-Sleep 1 }');
		assert.ok(!rendered.element.innerHTML.includes('katex'));
		await assertSnapshot(rendered.element.innerHTML);
	});

	it('Should not render math when dollar sign is followed by word character', async () => {
		const rendered = await renderMarkdownWithKatex('The cost is $10dollars for this item');
		assert.ok(!rendered.element.innerHTML.includes('katex'));
		await assertSnapshot(rendered.element.innerHTML);
	});

	it('Should still render math with special characters around dollars', async () => {
		const rendered = await renderMarkdownWithKatex('Hello ($\\frac{1}{2}$) and [$x^2$] work fine');
		assert.ok(rendered.element.innerHTML.includes('katex'));
		await assertSnapshot(rendered.element.innerHTML);
	});

	it('Should still render math at start and end of line', async () => {
