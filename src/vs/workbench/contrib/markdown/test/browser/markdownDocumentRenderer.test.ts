...
describe('Markdown Document Renderer Test', () => {
	...
	it('Should remove images with relative paths by default', async () => {
	...
	it('Can enable images with relative paths using setting', async () => {
		const result = await renderMarkdownDocument('![alt](src/img.png)', extensionService, languageService, {
			sanitizerConfig: {
