...
describe('NotebookExecutionService', () => {
	...
	it('cell is not runnable when no kernel is selected', async () => {
	...
	it('cell is not runnable when kernel does not support the language', async () => {
		await withTestNotebook(
			[],
	...
	it('cell is runnable when kernel does support the language', async () => {
		await withTestNotebook(
			[],
			async (viewModel, textModel) => {
	...
	it('Completes unconfirmed executions', async function () {
