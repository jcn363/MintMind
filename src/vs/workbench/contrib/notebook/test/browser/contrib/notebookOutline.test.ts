...
describe('Notebook Outline', function () {
	...
	it('special characters in heading', async function () {
	...
	it('Notebook falsely detects "empty cells"', async function () {
		await withNotebookOutline([
	...
	it('Heading text defines entry label', async function () {
	...
	it('Notebook outline ignores markdown headings #115200', async function () {
		await withNotebookOutline([
			['## h2 \n# h1', 'md', CellKind.Markup]
		], OutlineTarget.OutlinePane, outline => {
			assert.ok(outline instanceof NotebookCellOutline);
	...
	it('Symbols for goto quickpick are pre-cached', async function () {
		await withNotebookOutline([
