...
describe('NotebookFileWorkingCopyModel', function () {
	...
	it('no transient output is send to serializer', async function () {
	...
	it('no transient metadata is send to serializer', async function () {
	...
	it('no transient cell metadata is send to serializer', async function () {
	...
	it('Notebooks with outputs beyond the size threshold will throw for backup snapshots', async function () {
	...
	it('Notebook model will not return a save delegate if the serializer has not been retreived', async function () {
		const notebook = instantiationService.createInstance(NotebookTextModel,
			'notebook',
			URI.file('test'),
			[{ cellKind: CellKind.Code, language: 'foo', mime: 'foo', source: 'foo', outputs: [], metadata: { foo: 123, bar: 456 } }],
