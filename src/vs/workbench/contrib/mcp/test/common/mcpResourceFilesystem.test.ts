...
describe('Workbench - MCP - ResourceFilesystem', () => {
	...
	it('stat throws FileNotFound for nonexistent resources', async () => {
		transport.setResponder('resources/read', msg => {
	...
	it('readdir throws when reading a file as directory', async () => {
		transport.setResponder('resources/read', msg => {
			const request = msg as { id: string | number; params: { uri: string } };
			return {
				id: request.id,
	...
	it('watch file emits change events', async () => {
	...
	it('throws error for write operations', async () => {
		const uri = URI.parse('mcp-resource://746573742D736572766572/custom/hello/file.txt');
