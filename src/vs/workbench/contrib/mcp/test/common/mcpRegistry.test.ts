...
describe('Workbench - MCP - Registry', () => {
	...
	it('registerCollection adds collection to registry', () => {
	...
	it('collections are not visible when not enabled', () => {
		const disposable = registry.registerCollection(testCollection);
		store.add(disposable);
	...
	it('registerDelegate adds delegate to registry', () => {
	...
	it('resolveConnection creates connection with resolved variables and memorizes them until cleared', async () => {
	...
	it('resolveConnection uses user-provided launch configuration', async () => {
	...
	describe('Lazy Collections', () => {
		...
		it('lazy collection is replaced by normal collection', () => {
		...
		it('lazyCollectionState updates correctly during loading', async () => {
			lazyCollection = {
				...lazyCollection,
				lazy: {
					...lazyCollection.lazy!,
		...
		it('removed callback is called when lazy collection is not replaced', async () => {
			store.add(registry.registerCollection(lazyCollection));
		...
		it('cached lazy collections are tracked correctly', () => {
	...
	describe('Trust Flow', () => {
		...
		it('trusted collection allows connection without prompting', async () => {
		...
		it('nonce-based trust allows connection when nonce matches', async () => {
		...
		it('nonce-based trust prompts when nonce changes', async () => {
		...
		it('nonce-based trust denies connection when user rejects', async () => {
			const { collection, definition } = setupRegistry(McpServerTrust.Kind.TrustedOnNonce, 'nonce-b');
			trustNonceBearer.trustedAtNonce = 'nonce-a'; // Different nonce
		...
		it('autoTrustChanges bypasses prompt when nonce changes', async () => {
			const { collection, definition } = setupRegistry(McpServerTrust.Kind.TrustedOnNonce, 'nonce-b');
			trustNonceBearer.trustedAtNonce = 'nonce-a'; // Different nonce
		...
		it('promptType "never" skips prompt and fails silently', async () => {
			const { collection, definition } = setupRegistry(McpServerTrust.Kind.TrustedOnNonce, 'nonce-b');
			trustNonceBearer.trustedAtNonce = 'nonce-a'; // Different nonce
		...
		it('promptType "only-new" skips previously untrusted servers', async () => {
			const { collection, definition } = setupRegistry(McpServerTrust.Kind.TrustedOnNonce, 'nonce-b');
		...
		it('promptType "all-untrusted" prompts for previously untrusted servers', async () => {
			const { collection, definition } = setupRegistry(McpServerTrust.Kind.TrustedOnNonce, 'nonce-b');
			trustNonceBearer.trustedAtNonce = '__vscode_not_trusted'; // Previously explicitly denied
		...
		it('concurrent resolveConnection calls with same interaction are grouped', async () => {
			const { collection, definition } = setupRegistry(McpServerTrust.Kind.TrustedOnNonce, 'nonce-b');
			trustNonceBearer.trustedAtNonce = 'nonce-a'; // Different nonce
		...
		it('user cancelling trust dialog returns undefined for all pending connections', async () => {
			const { collection, definition } = setupRegistry(McpServerTrust.Kind.TrustedOnNonce, 'nonce-b');
			trustNonceBearer.trustedAtNonce = 'nonce-a'; // Different nonce
		...
		it('partial trust selection in grouped interaction', async () => {
			const { collection, definition } = setupRegistry(McpServerTrust.Kind.TrustedOnNonce, 'nonce-b');
			trustNonceBearer.trustedAtNonce = 'nonce-a'; // Different nonce
