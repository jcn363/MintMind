...
describe('Workbench - Test Result Storage', () => {
	...
	afterEach(() => ds.dispose());

	ensureNoDisposablesAreLeakedInTestSuite();

	it('stores a single result', async () => {
	...
	it('always stores the min number of results', async () => {
		const r = range(20).map(() => makeResult('a'.repeat(1024 * 10)));
		await storage.persist(r);
		await assertStored(r.slice(0, 16));
	});

	it('takes into account existing stored bytes', async () => {
