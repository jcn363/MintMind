...
describe('Workbench - Test Service', () => {
	ensureNoDisposablesAreLeakedInTestSuite();

	describe('simplifyTestsToExecute', () => {
		const tree1 = {
			a: {
				b1: {
					c1: {
		...
		it('goes to common root 1', async () => {
			const c = await getInitializedMainTestCollection(makeSimpleStubTree(tree1));
		...
		it('goes to common root 2', async () => {
			const c = await getInitializedMainTestCollection(makeSimpleStubTree(tree1));
		...
		it('goes to common root 3', async () => {
			const c = await getInitializedMainTestCollection(makeSimpleStubTree(tree1));
		...
		it('goes to common root 4', async () => {
			const c = await getInitializedMainTestCollection(makeSimpleStubTree(tree1));
		...
		it('no-op divergent trees', async () => {
			const c = await getInitializedMainTestCollection(makeSimpleStubTree(tree1));
