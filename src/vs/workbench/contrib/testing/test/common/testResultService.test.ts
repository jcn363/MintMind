...
describe('Workbench - Test Results Service', () => {
	...
	describe('LiveTestResult', () => {
		it('is empty if no tests are yet present', async () => {
		...
		it('initially queues nothing', () => {
			assert.deepStrictEqual(getChangeSummary(), []);
		});

		it('initializes with the subtree of requested tests', () => {
			assert.deepStrictEqual(getLabelsIn(r.tests), ['a', 'aa', 'ab', 'root']);
		});

		it('initializes with valid counts', () => {
	...
	describe('service', () => {
		...
		it('pushes new result', () => {
			results.push(r);
			assert.deepStrictEqual(results.results, [r]);
		});

		it('serializes and re-hydrates', async () => {
			results.push(r);
		...
		it('clears results but keeps ongoing tests', async () => {
			results.push(r);
			r.markComplete();
		...
		it('keeps ongoing tests on top, restored order when done', async () => {
			results.push(r);
			const r2 = results.push(new LiveTestResult(
				'',
				false,
		...
		it('pushes hydrated results', async () => {
			results.push(r);
		...
		it('inserts in correct order 2', async () => {
			results.push(r);
			const hydrated1 = await makeHydrated();
			results.push(hydrated1);
	...
	describe('output controller', () => {
		it('reads live output ranges', async () => {
		...
		it('corrects offsets for marked ranges', async () => {
			const ctrl = new TaskRawOutput();
