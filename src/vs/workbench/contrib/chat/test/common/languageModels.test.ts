...
describe('LanguageModels', function () {
	...
	it('empty selector returns all', async function () {
	...
	it('selector with id works properly', async function () {
		const result1 = await languageModels.selectLanguageModels({ id: 'test-id-1' });
		assert.deepStrictEqual(result1.length, 1);
		assert.deepStrictEqual(result1[0], 'test-id-1');
	});

	it('no warning that a matching model was not found #213716', async function () {
	...
	it('sendChatRequest returns a response-stream', async function () {
	...
	it('when clause defaults to true when omitted', async function () {
...
describe('LanguageModels - When Clause', function () {
	...
	it('when clause filters vendors correctly', async function () {
	...
	it('when clause evaluates to true when context key is true', async function () {
		const vendors = languageModelsWithWhen.getVendors();
		assert.ok(vendors.some(v => v.vendor === 'conditional-vendor'), 'conditional-vendor should be visible when testKey is true');
	});

	it('when clause evaluates to false when context key is false', async function () {
		const vendors = languageModelsWithWhen.getVendors();
