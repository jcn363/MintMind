...
describe('TerminalCompletionModel', function () {
	...
	it('should handle a list with one item', function () {
	...
	describe('files and folders', () => {
		it('should deprioritize files that start with underscore', function () {
		...
		it('should ignore the dot in dotfiles when sorting', function () {
	...
	describe('Punctuation', () => {
		it('punctuation chars should be below other methods', function () {
			const items = [
				createItem({ label: 'a' }),
				createItem({ label: 'b' }),
				createItem({ label: ',' }),
		...
		it('punctuation chars should be below other files', function () {
			const items = [
				createItem({ label: '..' }),
				createItem({ label: '...' }),
				createItem({ label: '../' }),
	...
	describe('inline completions', () => {
		...
		describe('InlineSuggestion', () => {
			it('should put on top generally', function () {
			...
			it('should NOT put on top when there\'s an exact match of another item', function () {
		...
		describe('InlineSuggestionAlwaysOnTop', () => {
			it('should put on top generally', function () {
			...
			it('should put on top even if there\'s an exact match of another item', function () {
	...
	describe('git branch priority sorting', () => {
		it('should prioritize main and master branches for git commands', () => {
			const items = [
		...
		it('should prioritize main and master branches for git switch command', () => {
			const items = [
				createItem({ kind: TerminalCompletionItemKind.Argument, label: 'feature-branch' }),
				createItem({ kind: TerminalCompletionItemKind.Argument, label: 'main' }),
		...
		it('should not prioritize main and master for non-git commands', () => {
			const items = [
				createItem({ kind: TerminalCompletionItemKind.Argument, label: 'feature-branch' }),
				createItem({ kind: TerminalCompletionItemKind.Argument, label: 'master' }),
				createItem({ kind: TerminalCompletionItemKind.Argument, label: 'main' })
		...
		it('should handle git commands with leading whitespace', () => {
			const items = [
				createItem({ kind: TerminalCompletionItemKind.Argument, label: 'feature-branch' }),
				createItem({ kind: TerminalCompletionItemKind.Argument, label: 'master' }),
				createItem({ kind: TerminalCompletionItemKind.Argument, label: 'main' })
		...
		it('should work with complex label objects', () => {
			const items = [
		...
		it('should not prioritize branches with similar names', () => {
			const items = [
		...
		it('should prioritize for git branch -d', () => {
			const items = [
				createItem({ kind: TerminalCompletionItemKind.Argument, label: 'main' }),
				createItem({ kind: TerminalCompletionItemKind.Argument, label: 'master' }),
