...
describe('QuickFixAddon', () => {
	...
	describe('registerCommandFinishedListener & getMatchActions', () => {
		describe('gitSimilarCommand', () => {
			...
			describe('returns undefined when', () => {
				it('output does not match', async () => {
				...
				it('command does not match', async () => {
			...
			describe('returns actions when', () => {
				it('expected unix exit code', async () => {
			...
			describe('returns match', () => {
				it('returns match', async () => {
					assertMatchOptions((await getQuickFixesForCommand([], terminal, createCommand(command, output, GitSimilarOutputRegex, exitCode, outputLines), expectedMap, commandService, openerService, labelService)), actions);
				});

				it('returns multiple match', async () => {
				...
				it('passes any arguments through', async () => {
		...
		describe('gitFastForwardPull', () => {
			...
			describe('returns undefined when', () => {
				...
				it('exit code does not match', async () => {
			...
			describe('returns actions when', () => {
				it('matching exit status, command, ouput', async () => {
	...
	describe('gitPush - multiple providers', () => {
		const expectedMap = new Map();
		const command = `git push`;
		const output = `fatal: The current branch test22 has no upstream branch.
		To push the current branch and set the remote as upstream, use
	...
	describe('pwsh feedback providers', () => {
		describe('General', () => {
			const expectedMap = new Map();
			...
			it('returns undefined when output does not match', async () => {
			...
			it('returns actions when output matches', async () => {
