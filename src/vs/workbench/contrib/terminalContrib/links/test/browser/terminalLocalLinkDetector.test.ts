...
describe('Workbench - TerminalLocalLinkDetector', () => {
	...
	describe('platform independent', () => {
		...
		it('should support multiple link results', async () => {
			validResources = [
		...
		it('should support trimming extra quotes', async () => {
		...
		it('should support trimming extra square brackets', async () => {
			validResources = [URI.file('/parent/cwd/foo')];
			fileService.setFiles(validResources);
			await assertLinks(TerminalBuiltinLinkType.LocalFile, '"foo]" on line 5', [
				{ range: [[1, 1], [16, 1]], uri: URI.file('/parent/cwd/foo') }
		...
		it('should support finding links after brackets', async () => {
			validResources = [URI.file('/parent/cwd/foo')];
			fileService.setFiles(validResources);
		...
		for (const l of unixLinks) {
			...
			describe(`Link: ${baseLink}`, () => {
				for (let i = 0; i < supportedLinkFormats.length; i++) {
					...
					it(`should detect in "${formattedLink}"`, async () => {
	...
	if (isWindows) {
		describe('Windows', () => {
			...
			describe('WSL', () => {
				it('Unix -> Windows /mnt/ style links', async () => {
