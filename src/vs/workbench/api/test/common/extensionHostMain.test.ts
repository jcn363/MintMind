...
describe('ExtensionHostMain#ErrorHandler - Wrapping prepareStackTrace can cause slowdown and eventual stack overflow #184926 ', function () {
	...
	it('set/reset prepareStackTrace-callback', function () {
	...
	it('wrap prepareStackTrace-callback', function () {
	...
	it('prevent rewrapping', function () {
	...
	describe('https://gist.github.com/thecrypticace/f0f2e182082072efdaf0f8e1537d2cce', function () {

		it('Restored, separate operations', () => {
			// Actual Test
			let original;
		...
		it('Never restored, separate operations', () => {
			// Operation 1
			for (let i = 0; i < 12_500; ++i) { Error.prepareStackTrace = Error.prepareStackTrace; }
			assert.ok(new Error().stack);
		...
		it('Restored, too many uses before restoration', async () => {
			const original = Error.prepareStackTrace;
