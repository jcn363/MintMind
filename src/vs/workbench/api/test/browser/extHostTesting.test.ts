...
describe('ExtHost Testing', () => {
	...
	describe('OwnedTestCollection', () => {
		it('adds a root recursively', async () => {
		...
		it('can add an item with same ID as root', () => {
			single.collectDiff();
		...
		it('no-ops if items not changed', () => {
			single.collectDiff();
			assert.deepStrictEqual(single.collectDiff(), []);
		});

		it('watches property mutations', () => {
			single.expand(single.root.id, Infinity);
			single.collectDiff();
		...
		it('treats in-place replacement as mutation', () => {
			single.expand(single.root.id, Infinity);
			single.collectDiff();
		...
		describe('expandibility restoration', () => {
			...
			it('does not restore an unexpanded state', async () => {
			...
			it('restores resolve state on replacement', async () => {
				await single.expand(single.root.id, Infinity);
			...
			it('does not expand if new child is not expandable', async () => {
				await single.expand(single.root.id, Infinity);
				assert.deepStrictEqual(resolveCalls, [undefined, 'id-a']);
		...
		it('treats in-place replacement as mutation deeply', () => {
			single.expand(single.root.id, Infinity);
			single.collectDiff();
		...
		it('moves an item to be a new child', async () => {
			await single.expand(single.root.id, 0);
			single.collectDiff();
			const b = single.root.children.get('id-b') as TestItemImpl;
			const a = single.root.children.get('id-a') as TestItemImpl;
		...
		it('sends document sync events', async () => {
			await single.expand(single.root.id, 0);
			single.collectDiff();
	...
	describe('MirroredTestCollection', () => {
		...
		// 	it('mirrors creation of the root', () => {
		...
		// 		it('is a no-op if a node is added and removed', () => {
		...
		// 		it('is a single-op if a node is added and changed', () => {
	...
	describe('TestRunTracker', () => {
		...
		it('tracks a run started from a main thread request', () => {
		...
		it('run cancel force ends after a timeout', () => {
		...
		it('run cancel force ends on second cancellation request', () => {
			const tracker = ds.add(c.prepareForMainThreadTestRun(ext, req, dto, configuration, cts.token));
			const task = c.createTestRun(ext, 'ctrl', single, req, 'run1', true);
			const onEnded = sinon.stub();
			ds.add(tracker.onEnd(onEnded));
		...
		it('tracks a run started from an extension request', () => {
		...
		it('guards calls after runs are ended', () => {
		...
		it('sets state of test with identical local IDs (#131827)', () => {
	...
	describe('service', () => {
		...
		it('exposes active profiles correctly', async () => {
