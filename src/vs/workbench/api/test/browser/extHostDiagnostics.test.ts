...
describe('ExtHostDiagnostics', () => {
	...
	it('diagnostic collection, forEach, clear, has', function () {
	...
	it('diagnostic collection, immutable read', function () {
		const collection = new DiagnosticCollection('test', 'test', 100, 100, versionProvider, extUri, new DiagnosticsShape(), new Emitter());
		collection.set(URI.parse('foo:bar'), [
			new Diagnostic(new Range(0, 0, 1, 1), 'message-1'),
			new Diagnostic(new Range(0, 0, 1, 1), 'message-2')
	...
	it('diagnostics collection, set with dupliclated tuples', function () {
		const collection = new DiagnosticCollection('test', 'test', 100, 100, versionProvider, extUri, new DiagnosticsShape(), new Emitter());
	...
	it('diagnostics collection, set tuple overrides, #11547', function () {
	...
	it('do send message when not making a change', function () {
	...
	it('diagnostics collection, tuples and undefined (small array), #15585', function () {
	...
	it('diagnostics collection, tuples and undefined (large array), #15585', function () {
	...
	it('diagnostic capping (max per file)', function () {
	...
	it('diagnostic capping (max files)', function () {
	...
	it('diagnostic eventing', async function () {
	...
	it('vscode.languages.onDidChangeDiagnostics Does Not Provide Document URI #49582', async function () {
		const emitter = new Emitter<readonly URI[]>();
		const collection = new DiagnosticCollection('ddd', 'test', 100, 100, versionProvider, extUri, new DiagnosticsShape(), emitter);
	...
	it('diagnostics with related information', function (done) {
	...
	it('vscode.languages.getDiagnostics appears to return old diagnostics in some circumstances #54359', function () {
	...
	it('Error updating diagnostics from extension #60394', function () {
		let callCount = 0;
		const collection = new DiagnosticCollection('ddd', 'test', 100, 100, versionProvider, extUri, new class extends DiagnosticsShape {
			override $changeMany(owner: string, entries: [UriComponents, IMarkerData[]][]) {
				callCount += 1;
	...
	it('getDiagnostics does not tolerate sparse diagnostic arrays', function () {
		const diags = new ExtHostDiagnostics(new class implements IMainContext {
			getProxy(): any {
	...
	it('Version id is set whenever possible', function () {
	...
	it('Diagnostics created by tasks aren\'t accessible to extensions #47292', async function () {
	...
	it('languages.getDiagnostics doesn\'t handle case insensitivity correctly #128198', function () {
