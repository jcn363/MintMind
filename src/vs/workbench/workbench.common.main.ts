...
import { loadEditorContributions } from '../editor/editor.all.js';
import './api/browser/extensionHost.contribution.js';
import './browser/workbench.contribution.js';

// Load editor contributions asynchronously
loadEditorContributions();
...
const lazyLoadContributions = async () => {
	// Lazy load heavy contributions on demand with priority grouping
	const criticalContributions = [
		() => import('./contrib/telemetry/browser/telemetry.contribution.js'),
	];

	const highPriorityContributions = [
		() => import('./contrib/extensions/browser/extensions.contribution.js'),
		() => import('./contrib/search/browser/search.contribution.js'),
		() => import('./contrib/search/browser/searchView.js'),
	];

	const mediumPriorityContributions = [
	...
	const lowPriorityContributions = [
		() => import('./contrib/chat/browser/chat.contribution.js'),
		() => import('./contrib/inlineChat/browser/inlineChat.contribution.js'),
	];

	// Load critical contributions immediately
	await Promise.all(criticalContributions.map(fn => fn()));

	// Progressive loading based on user interaction and idle time
	const loadContributions = async (contributions: (() => Promise<unknown>)[], delay: number = 0) => {
		if (delay > 0) {
			await new Promise(resolve => setTimeout(resolve, delay));
		}

		// Load in smaller batches with micro-delays to prevent blocking
		const batchSize = 2;
	...
	const triggerProgressiveLoading = () => {
		// High priority after 200ms
		loadContributions(highPriorityContributions, 200);

		// Medium priority after 800ms
		loadContributions(mediumPriorityContributions, 800);

		// Low priority after 2s or on idle
		const loadLowPriority = () => loadContributions(lowPriorityContributions, 0);
		if (typeof requestIdleCallback !== 'undefined') {
			requestIdleCallback(loadLowPriority, { timeout: 2000 });
		} else {
			setTimeout(loadLowPriority, 2000);
		}
	};

	// Start progressive loading immediately for critical interactions
	triggerProgressiveLoading();
};

// Start lazy loading with improved timing
lazyLoadContributions();
...
// MCP and Chat Context (keep these as they might be critical)
...
// Explorer (keep core file operations)
...
// Debug (keep editor contributions as they might be needed)
...
// Extensions Management (keep viewlet for core functionality)
