...
describe('AuthenticationAccessService', () => {
	...
	describe('isAccessAllowed', () => {
		it('returns undefined for unknown extension with no product configuration', () => {
			const result = authenticationAccessService.isAccessAllowed('github', 'user@example.com', 'unknown-extension');
			assert.strictEqual(result, undefined);
		});

		it('returns true for trusted extension from product.json (array format)', () => {
		...
		it('returns true for trusted extension from product.json (object format)', () => {
		...
		it('returns undefined for extension not in trusted list', () => {
		...
		it('returns stored allowed state when extension is in storage', () => {
		...
		it('returns true for extension in storage with allowed=true', () => {
			authenticationAccessService.updateAllowedExtensions('github', 'user@example.com', [{
		...
		it('returns true for extension in storage with undefined allowed property (legacy behavior)', () => {
		...
		it('product.json trusted extensions take precedence over storage', () => {
	...
	describe('readAllowedExtensions', () => {
		...
		it('includes trusted extensions from product.json (array format)', () => {
			productService.trustedExtensionAuthAccess = ['trusted-extension-1', 'trusted-extension-2'];
		...
		it('includes trusted extensions from product.json (object format)', () => {
			productService.trustedExtensionAuthAccess = {
				'github': ['github-extension'],
				'microsoft': ['microsoft-extension']
			};
		...
		it('merges stored extensions with trusted extensions from product.json', () => {
			productService.trustedExtensionAuthAccess = ['trusted-extension'];
		...
		it('updates existing stored extension to trusted when found in product.json', () => {
	...
	describe('updateAllowedExtensions', () => {
		it('adds new extensions to storage', () => {
			const extensions: AllowedExtension[] = [
				{ id: 'extension1', name: 'Extension 1', allowed: true },
				{ id: 'extension2', name: 'Extension 2', allowed: false }
			];
		...
		it('updates existing extension allowed status', () => {
		...
		it('updates existing extension name when new name is provided', () => {
		...
		it('does not store trusted extensions - they should only come from product.json', () => {
			productService.trustedExtensionAuthAccess = ['trusted-extension'];
		...
		it('filters out trusted extensions before storing', () => {
		...
		it('fires onDidChangeExtensionSessionAccess event', () => {
	...
	describe('removeAllowedExtensions', () => {
		it('removes all extensions from storage', () => {
		...
		it('does not affect trusted extensions from product.json', () => {
			productService.trustedExtensionAuthAccess = ['trusted-extension'];
	...
	describe('integration with product.json configurations', () => {
		it('handles switching between array and object format', () => {
		...
		it('handles empty trusted extension configurations', () => {
			// Test undefined
			productService.trustedExtensionAuthAccess = undefined;
			let result = authenticationAccessService.readAllowedExtensions('github', 'user@example.com');
			assert.strictEqual(result.length, 0);
