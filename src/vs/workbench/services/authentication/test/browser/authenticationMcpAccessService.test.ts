...
describe('AuthenticationMcpAccessService', () => {
	...
	describe('isAccessAllowed', () => {
		it('returns undefined for unknown MCP server with no product configuration', () => {
			const result = authenticationMcpAccessService.isAccessAllowed('github', 'user@example.com', 'unknown-server');
			assert.strictEqual(result, undefined);
		});

		it('returns true for trusted MCP server from product.json (array format)', () => {
		...
		it('returns true for trusted MCP server from product.json (object format)', () => {
		...
		it('returns undefined for MCP server not in trusted list', () => {
		...
		it('returns stored allowed state when server is in storage', () => {
		...
		it('returns true for server in storage with allowed=true', () => {
			authenticationMcpAccessService.updateAllowedMcpServers('github', 'user@example.com', [{
				id: 'allowed-server',
				name: 'Allowed Server',
				allowed: true
		...
		it('returns true for server in storage with undefined allowed property (legacy behavior)', () => {
		...
		it('product.json trusted servers take precedence over storage', () => {
	...
	describe('readAllowedMcpServers', () => {
		...
		it('includes trusted servers from product.json (array format)', () => {
			productService.trustedMcpAuthAccess = ['trusted-server-1', 'trusted-server-2'];
		...
		it('includes trusted servers from product.json (object format)', () => {
			productService.trustedMcpAuthAccess = {
				'github': ['github-server'],
				'microsoft': ['microsoft-server']
			};
		...
		it('merges stored servers with trusted servers from product.json', () => {
			productService.trustedMcpAuthAccess = ['trusted-server'];
		...
		it('updates existing stored server to be trusted when it appears in product.json', () => {
		...
		it('handles non-array product.json configuration gracefully', () => {
	...
	describe('updateAllowedMcpServers', () => {
		it('stores new MCP servers', () => {
			const servers: AllowedMcpServer[] = [
				{ id: 'server1', name: 'Server 1', allowed: true },
				{ id: 'server2', name: 'Server 2', allowed: false }
			];
		...
		it('updates existing MCP server allowed status', () => {
			// First add a server
			authenticationMcpAccessService.updateAllowedMcpServers('github', 'user@example.com', [
				{ id: 'server1', name: 'Server 1', allowed: true }
			]);
		...
		it('updates existing MCP server name when new name is provided', () => {
		...
		it('does not update name when new name is same as ID', () => {
		...
		it('adds new servers while preserving existing ones', () => {
		...
		it('does not store trusted servers from product.json', () => {
			productService.trustedMcpAuthAccess = ['trusted-server'];
		...
		it('fires onDidChangeMcpSessionAccess event', () => {
	...
	describe('removeAllowedMcpServers', () => {
		it('removes all stored MCP servers for account', () => {
		...
		it('does not affect trusted servers from product.json', () => {
			productService.trustedMcpAuthAccess = ['trusted-server'];
		...
		it('handles removal of non-existent data gracefully', () => {
	...
	describe('onDidChangeMcpSessionAccess event', () => {
		it('event is fired for each update operation', () => {
	...
	describe('integration scenarios', () => {
		it('complete workflow: add, update, query, remove', () => {
		...
		it('multiple providers and accounts are isolated', () => {
		...
		it('product.json configuration takes precedence in all scenarios', () => {
			productService.trustedMcpAuthAccess = {
		...
		it('handles edge cases with empty or null values', () => {
