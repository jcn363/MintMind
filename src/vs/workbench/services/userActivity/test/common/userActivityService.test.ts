...
describe('UserActivityService', () => {
	...
	it('isActive should be true initially', () => {
		assert.ok(userActivityService.isActive);
	});

	it('markActive should be inactive when all handles gone', () => {
	...
	it('markActive sets active whenHeldFor', async () => {
	...
	it('markActive whenHeldFor before triggers', async () => {
		userActivityService.markActive().dispose();
		clock.tick(MARK_INACTIVE_DEBOUNCE);
	...
	it('markActive with extendOnly only extends if already active', () => {
		// Make user inactive
		userActivityService.markActive().dispose();
		clock.tick(MARK_INACTIVE_DEBOUNCE);
		assert.strictEqual(userActivityService.isActive, false);
