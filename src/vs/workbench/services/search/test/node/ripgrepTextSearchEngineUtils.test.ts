...
describe('RipgrepTextSearchEngine', () => {
	...
	it('unicodeEscapesToPCRE2', async () => {
	...
	it('fixRegexNewline - re', () => {
		function testFixRegexNewline([inputReg, testStr, shouldMatch]: readonly [string, string, boolean]): void {
			...
			assert.strictEqual(reg.it(testStr), shouldMatch, `${inputReg} => ${reg}, ${testStr}, ${shouldMatch}`);
		}
	...
	describe('RipgrepParser', () => {
		...
		it('multiple submatches without newline in between (#131507)', () => {
			testParser(
				[
		...
		it('multiple submatches with newline in between (#131507)', () => {
			testParser(
				[
	...
	it('brace expansion for ripgrep', () => {
