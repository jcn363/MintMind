...
describe('EditorsObserver', function () {
	...
	it('basics (single group)', async () => {
		await testSingleGroupBasics();
	});

	it('basics (single group, scoped)', async () => {
	...
	it('hasEditor/hasEditors - same resource, different type id', async () => {
		const [part, observer] = await createEditorObserver();
	...
	it('hasEditor/hasEditors - side by side editor support', async () => {
	...
	it('initial editors are part of observer and state is persisted & restored (single group)', async () => {
	...
	it('initial editors are part of observer (multi group)', async () => {
		const [part] = await createPart();
	...
	it('observer does not restore editors that cannot be serialized', async () => {
		const [part] = await createPart();
	...
	it('observer closes editors when limit reached (across all groups)', async () => {
		const [part] = await createPart();
	...
	it('observer closes editors when limit reached (in group)', async () => {
		const [part] = await createPart();
	...
	it('observer does not close sticky', async () => {
		const [part] = await createPart();
		disposables.add(part.enforcePartOptions({ limit: { enabled: true, value: 3 } }));
	...
	it('observer does not close scratchpads', async () => {
		const [part] = await createPart();
		disposables.add(part.enforcePartOptions({ limit: { enabled: true, value: 3 } }));
