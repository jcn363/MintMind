...
describe('StoredFileWorkingCopy (with custom save)', function () {
	...
	it('save (custom implemented)', async () => {
		let savedCounter = 0;
	...
	it('save cancelled (custom implemented)', async () => {
		let savedCounter = 0;
		let lastSaveEvent: IStoredFileWorkingCopySaveEvent | undefined = undefined;
		disposables.add(workingCopy.onDidSave(e => {
			savedCounter++;
...
describe('StoredFileWorkingCopy', function () {
	...
	it('registers with working copy service', async () => {
	...
	it('dirty - working copy marks non-dirty when undo reaches saved version ID', async () => {
		await workingCopy.resolve();
	...
	it('resolve (without backup)', async () => {
	...
	it('resolve (with backup, preserves metadata and orphaned state)', async () => {
		return runWithFakedTimers({}, async () => {
			await workingCopy.resolve({ contents: bufferToStream(VSBuffer.fromString('hello backup')) });
	...
	it('resolve (updates orphaned state accordingly)', async () => {
		return runWithFakedTimers({}, async () => {
			await workingCopy.resolve();
	...
	it('resolve (FILE_NOT_MODIFIED_SINCE can be handled for resolved working copies)', async () => {
		await workingCopy.resolve();
	...
	it('resolve (FILE_NOT_MODIFIED_SINCE still updates readonly state)', async () => {
	...
	it('resolve does not alter content when model content changed in parallel', async () => {
		await workingCopy.resolve();
	...
	it('save (no errors) - simple', async () => {
		let savedCounter = 0;
		let lastSaveEvent: IStoredFileWorkingCopySaveEvent | undefined = undefined;
		disposables.add(workingCopy.onDidSave(e => {
			savedCounter++;
	...
	it('save (no errors) - save reason', async () => {
		let savedCounter = 0;
		let lastSaveEvent: IStoredFileWorkingCopySaveEvent | undefined = undefined;
		disposables.add(workingCopy.onDidSave(e => {
			savedCounter++;
	...
	it('save (no errors) - multiple', async () => {
		let savedCounter = 0;
		disposables.add(workingCopy.onDidSave(e => {
			savedCounter++;
		}));
	...
	it('save (no errors) - multiple, cancellation', async () => {
		let savedCounter = 0;
		disposables.add(workingCopy.onDidSave(e => {
			savedCounter++;
		}));
	...
	it('save (no errors) - not forced but not dirty', async () => {
		let savedCounter = 0;
		disposables.add(workingCopy.onDidSave(e => {
			savedCounter++;
		}));
	...
	it('save (no errors) - forced but not dirty', async () => {
		let savedCounter = 0;
		disposables.add(workingCopy.onDidSave(e => {
			savedCounter++;
		}));
	...
	it('save (no errors) - save clears orphaned', async () => {
		return runWithFakedTimers({}, async () => {
			let savedCounter = 0;
			disposables.add(workingCopy.onDidSave(e => {
				savedCounter++;
	...
	it('save (errors, bubbles up with `ignoreErrorHandler`)', async () => {
		await workingCopy.resolve();
	...
	it('Save Participant, calling save from within is unsupported but does not explode (sync save)', async function () {
		await workingCopy.resolve();

		await testSaveFromSaveParticipant(workingCopy, false);
	});

	it('Save Participant, calling save from within is unsupported but does not explode (async save)', async function () {
		await workingCopy.resolve();
	...
	it('isReadonly, isResolved, dispose, isDisposed', async () => {
	...
	it('readonly change event', async () => {
