...
describe('UntitledFileWorkingCopyManager', () => {
	...
	it('dirty - scratchpads are never dirty', async () => {
		let dirtyCounter = 0;
		disposables.add(manager.untitled.onDidChangeDirty(e => {
			dirtyCounter++;
		}));
	...
	it('resolve - with initial value', async () => {
		let dirtyCounter = 0;
		disposables.add(manager.untitled.onDidChangeDirty(e => {
			dirtyCounter++;
		}));
	...
	it('resolve - with initial value but markDirty: false', async () => {
		let dirtyCounter = 0;
		disposables.add(manager.untitled.onDidChangeDirty(e => {
			dirtyCounter++;
		}));
	...
	it('resolve begins counter from 1 for disposed untitled', async () => {
	...
	it('resolve - untitled resource used for new working copy', async () => {
	...
	it('resolve - with associated resource', async () => {
	...
	it('save - without associated resource', async () => {
	...
	it('save - with associated resource', async () => {
		let savedEvent: { source: URI; target: URI } | undefined = undefined;
		disposables.add(manager.untitled.onDidSave(e => {
			savedEvent = e;
		}));
	...
	it('save - with associated resource (asks to overwrite)', async () => {
		const workingCopy = await manager.untitled.resolve({ associatedResource: { path: '/some/associated.txt' } });
		workingCopy.model?.updateContents('Simple Save with associated resource');
	...
	it('manager with different types produce different URIs', async () => {
		try {
			manager = disposables.add(new FileWorkingCopyManager(
	...
	it('manager without typeId produces backwards compatible URIs', async () => {
		try {
			manager = disposables.add(new FileWorkingCopyManager(
				NO_TYPE_ID,
				new TestStoredFileWorkingCopyModelFactory(),
