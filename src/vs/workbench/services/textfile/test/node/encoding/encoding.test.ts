...
describe('Encoding', () => {
	...
	it('detectBOM does not return error for non existing file', async () => {
	...
	it('detectBOM ANSI', async function () {
	...
	it('detectBOM ANSI (2)', async function () {
	...
	it('detectEncodingFromBuffer (JSON saved as PNG)', async function () {
	...
	it('detectEncodingFromBuffer (PNG saved as TXT)', async function () {
	...
	it('detectEncodingFromBuffer (XML saved as PNG)', async function () {
	...
	it('detectEncodingFromBuffer (QWOFF saved as TXT)', async function () {
	...
	it('detectEncodingFromBuffer (CSS saved as QWOFF)', async function () {
	...
	it('detectEncodingFromBuffer (PDF)', async function () {
	...
	it('detectEncodingFromBuffer (guess UTF-16 LE from content without BOM)', async function () {
	...
	it('detectEncodingFromBuffer (guess UTF-16 BE from content without BOM)', async function () {
	...
	it('autoGuessEncoding (UTF8)', async function () {
	...
	it('autoGuessEncoding (ASCII)', async function () {
		const file = FileAccess.asFileUri('vs/workbench/services/textfile/test/node/encoding/fixtures/some_ansi.css').fsPath;
		const buffer = await readExactlyByFile(file, 512 * 8);
		const mimes = await encoding.detectEncodingFromBuffer(buffer, true);
	...
	it('autoGuessEncoding (ShiftJIS)', async function () {
	...
	it('autoGuessEncoding (CP1252)', async function () {
	...
	it('autoGuessEncoding (candidateGuessEncodings - ShiftJIS)', async function () {
	...
	it('toDecodeStream - some stream', async function () {
	...
	it('toDecodeStream - some stream, expect too much data', async function () {
		const source = newTestReadableStream([
			Buffer.from([65, 66, 67]),
			Buffer.from([65, 66, 67]),
			Buffer.from([65, 66, 67]),
	...
	it('toDecodeStream - some stream, no data', async function () {
	...
	it('toDecodeStream - encoding, utf16be', async function () {
	...
	it('toDecodeStream - empty file', async function () {
	...
	it('toDecodeStream - decodes buffer entirely', async function () {
	...
	it('toDecodeStream - some stream (GBK issue #101856)', async function () {
	...
	it('toDecodeStream - some stream (UTF-8 issue #102202)', async function () {
	...
	it('toDecodeStream - binary', async function () {
		const source = () => {
	...
	it('toEncodeReadable - encoding, utf16be', async function () {
		const path = FileAccess.asFileUri('vs/workbench/services/textfile/test/node/encoding/fixtures/some_utf16be.css').fsPath;
	...
	it('toEncodeReadable - empty readable to utf8', async function () {
	...
	[{
		...
		it(`toEncodeReadable - empty readable to ${utfEncoding} with BOM`, async function () {
			const source: streams.Readable<string> = {
				read() {
					return null;
				}
	...
	it('encodingExists', async function () {
