...
describe('KeybindingsEditing', () => {
	...
	it('errors cases - parse errors', async () => {
	...
	it('errors cases - parse errors 2', async () => {
	...
	it('errors cases - did not find an array', async () => {
	...
	it('edit a default keybinding to an empty file', async () => {
		await fileService.writeFile(userDataProfileService.currentProfile.keybindingsResource, VSBuffer.fromString(''));
	...
	it('edit a default keybinding to an empty array', async () => {
	...
	it('edit a default keybinding in an existing array', async () => {
	...
	it('add another keybinding', async () => {
	...
	it('edit an user keybinding with more than one element', async () => {
	...
	it('remove a default keybinding', async () => {
		const expected: IUserFriendlyKeybinding[] = [{ key: 'alt+c', command: '-a' }];
	...
	it('remove a default keybinding should not ad duplicate entries', async () => {
		const expected: IUserFriendlyKeybinding[] = [{ key: 'alt+c', command: '-a' }];
		await testObject.removeKeybinding(aResolvedKeybindingItem({ command: 'a', firstChord: { keyCode: KeyCode.KeyC, modifiers: { altKey: true } } }));
		await testObject.removeKeybinding(aResolvedKeybindingItem({ command: 'a', firstChord: { keyCode: KeyCode.KeyC, modifiers: { altKey: true } } }));
		await testObject.removeKeybinding(aResolvedKeybindingItem({ command: 'a', firstChord: { keyCode: KeyCode.KeyC, modifiers: { altKey: true } } }));
	...
	it('remove a user keybinding', async () => {
	...
	it('reset an edited keybinding', async () => {
		await writeToKeybindingsFile({ key: 'alt+c', command: 'b' });
	...
	it('add a new keybinding to unassigned keybinding', async () => {
	...
	it('update command and when expression', async () => {
	...
	it('update when expression', async () => {
	...
	it('remove when expression', async () => {
		await writeToKeybindingsFile({ key: 'alt+c', command: '-a', when: 'editorTextFocus && !editorReadonly' });
