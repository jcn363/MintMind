...
describe('TextResourceEditorInput', () => {
	...
	it('basics', async () => {
		...
		accessor.modelService.createModel('function it() {}', accessor.languageService.createById(PLAINTEXT_LANGUAGE_ID), resource);
		...
		assert.ok(model);
		assert.strictEqual(snapshotToString(((model as TextResourceEditorModel).createSnapshot()!)), 'function it() {}');
	});

	it('preferred language (via ctor)', async () => {
	...
	it('preferred language (via setPreferredLanguageId)', async () => {
		const registration = accessor.languageService.registerLanguage({
			id: 'resource-input-test',
		});
	...
	it('preferred contents (via ctor)', async () => {
		const resource = URI.from({ scheme: 'inmemory', authority: null!, path: 'thePath' });
		accessor.modelService.createModel('function it() {}', accessor.languageService.createById(PLAINTEXT_LANGUAGE_ID), resource);
	...
	it('preferred contents (via setPreferredContents)', async () => {
		const resource = URI.from({ scheme: 'inmemory', authority: null!, path: 'thePath' });
		accessor.modelService.createModel('function it() {}', accessor.languageService.createById(PLAINTEXT_LANGUAGE_ID), resource);
