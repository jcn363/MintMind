...
/**
 * A listener for URLs that are opened from the OS and handled by MintMind.
 * Depending on the platform, this works differently:
 * - Windows: we use `app.setAsDefaultProtocolClient()` to register MintMindnd with the OS
 ...
 *            that calls MintMindnd with the `open-url` command line argument
