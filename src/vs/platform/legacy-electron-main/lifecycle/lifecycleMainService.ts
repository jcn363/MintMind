...
export class LifecycleMainService extends Disposable implements ILifecycleMainService {
	...
	private async onBeforeUnloadWindowInRenderer(window: ICodeWindow, reason: UnloadReason): Promise<boolean /* veto */> {
			...
			if (isTauriMode()) {
				try {
					(async () => {
						...
						await emitTo(window.label, 'vscode:onBeforeUnload', { okChannel, cancelChannel, reason });
						const okUnlisten = await listen(okChannel, () => { resolve(false); okUnlisten(); cancelUnlisten(); });
						const cancelUnlisten = await listen(cancelChannel, () => { resolve(true); okUnlisten(); cancelUnlisten(); });
					})();
				} catch (error) {
					this.logService.error(error);
					resolve(false); // Safe resolution without Electron IPC
				}
			} else {
				validatedIpcMain.once(okChannel, () => resolve(false));
				validatedIpcMain.once(cancelChannel, () => resolve(true));
				window.send('vscode:onBeforeUnload', { okChannel, cancelChannel, reason });
			}
		});
	}

	private async onWillUnloadWindowInRenderer(window: ICodeWindow, reason: UnloadReason): Promise<void> {
		return new Promise<void>(resolve => {
			...
			if (isTauriMode()) {
				try {
					(async () => {
						const { listen, emitTo } = await import('@tauri-apps/api/event');
						await emitTo(window.label, 'vscode:onWillUnload', { replyChannel, reason });
						const unlisten = await listen(replyChannel, () => { resolve(); unlisten(); });
					})();
				} catch (error) {
					this.logService.error(error);
					resolve(); // Safe resolution without Electron IPC
				}
			} else {
