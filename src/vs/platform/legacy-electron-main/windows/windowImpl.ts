...
import { WebviewWindow, PhysicalPosition, PhysicalSize } from '@tauri-apps/api/webviewWindow';
import { Monitor } from '@tauri-apps/api/monitor';
import { release } from 'os';
import { DeferredPromise, Delayer, RunOnceScheduler, timeout } from '../../../base/common/async.js';
...
import { DEFAULT_CUSTOM_TITLEBAR_HEIGHT, IFolderToOpen, INativeWindowConfiguration, IWindowSettings, IWorkspaceToOpen, MenuBarVisibility, MenuSettings, TitlebarStyle, getMenuBarVisibility, hasNativeTitlebar, useNativeFullScreen, useWindowControlsOverlay } from '../../window/common/window.js';
import { IBaseWindow, ICodeWindow, ILoadEvent, IWindowState, LoadReason, WindowError, WindowMode, defaultWindowState } from '../../window/electron-main/window.js';
...
import { IWindowsMainService, OpenContext, WindowStateValidator, defaultBrowserWindowOptions, getAllWindowsExcludingOffscreen } from './windows.js';
...
export abstract class BaseWindow extends Disposable implements IBaseWindow {
	...
	protected setWin(win: WebviewWindow, options?: any): void {
		this._win = win;
	...
	setRepresentedFilename(filename: string): void {
		// Tauri doesn't have direct equivalent for represented filename
		// We'll store it locally for compatibility
	...
	updateWindowControls(options: { height?: number; backgroundColor?: string; foregroundColor?: string }): void {
		...
		// Tauri window controls - simplified approach
		// Note: Tauri handles window controls differently, this is a basic implementation
		if (options.height) {
			// Store the height for potential future use
			// Tauri may not need explicit window control positioning like Electron
		}
	}
	...
	get isFullScreen(): boolean {
		...
		// Tauri fullscreen check
		try {
			const isFullscreen = win.isFullscreen();
			return isFullscreen;
		} catch (error) {
			this.logService.error('Failed to check fullscreen state:', error);
			return false;
		}
	}
...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	private updateConfiguration(configuration: INativeWindowConfiguration, options: ILoadOptions): void {
		...
		// (as indicated by MINTMIND_CLI environment), make sure to
