...
try {
	if (typeof window !== 'undefined' && (window as any).__TAURI__) {
		...
		getAllWindows = tauriWindow.getAllWindows;
	}
} catch (error) {
...
export class WindowsMainService extends Disposable implements IWindowsMainService {
	...
	private async createTauriWindow(options: IOpenBrowserWindowOptions): Promise<ICodeWindow> {
		if (!isTauriRuntime || !WebviewWindow) {
			throw new Error('Tauri runtime not available');
		}

		try {
			const windowConfig = this.convertToTauriWindowConfig(options);
			const label = await invoke<string>('create_window', { config: windowConfig });
			const tauriWindow = await WebviewWindow.getByLabel(label);

			// Create Tauri window instance
			const window = this.instantiationService.createInstance(TauriCodeWindow, {
				state: options.windowToUse?.windowState || defaultWindowState(),
				extensionDevelopmentPath: options.cli?.extensionDevelopmentPath
			}, this.logService, this.loggerService, this.environmentMainService, this.policyService, this.userDataProfilesService, this.fileService, this.applicationStorageMainService, this.storageMainService, this.configurationService, this.themeMainService, this.workspacesManagementMainService, this.backupMainService, this.telemetryService, this.dialogMainService, this.lifecycleMainService, this.productService, this.protocolMainService, this, this.stateService, this.instantiationService);

			// Set the Tauri window
			(window as any).setTauriWindow(tauriWindow);
	...
	private convertToTauriWindowConfig(options: IOpenBrowserWindowOptions): any {
		const windowConfig = {
			label: `window-${Date.now()}`,
			title: this.productService.nameLong,
			width: options.windowToUse?.windowState?.width || 1024,
			height: options.windowToUse?.windowState?.height || 768,
			x: options.windowToUse?.windowState?.x,
			y: options.windowToUse?.windowState?.y,
			minWidth: 400,
			minHeight: 300,
			resizable: true,
			maximizable: true,
			minimizable: true,
			closable: true,
			center: !options.windowToUse?.windowState?.x && !options.windowToUse?.windowState?.y,
			decorations: true,
		};
	...
	private async onUNCHostNotAllowed(path: string, options: IPathResolveOptions): Promise<IPathToOpen<ITextEditorOptions> | undefined> {
		...
		if (response === 2) {
			try {
				await invoke('open_external', { url: 'https://aka.ms/vscode-windows-unc', default_application: undefined });
			} catch (error) {
				this.logService.error('[WindowsMainService]: Failed to open UNC help URL', error);
			}
