...
export function getAllWindowsExcludingOffscreen() {
	if (isTauriMode()) {
		// In Tauri, get all windows from the WebviewWindow API
		return WebviewWindow.getAll().filter(win => !win.isMinimized()); // Tauri doesn't have isOffscreen, using isMinimized as proxy
	} else {
