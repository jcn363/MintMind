...
export class SharedProcess extends Disposable {
	...
	private async registerListeners(): Promise<void> {
		...
		if (isTauriMode()) {
			...
			const channelUnlisten = await listen(SharedProcessChannelConnection.request, (event) => {
				this.onWindowConnection(event, event.payload.nonce, SharedProcessChannelConnection.response);
			});
			const rawUnlisten = await listen(SharedProcessRawConnection.request, (event) => {
				this.onWindowConnection(event, event.payload.nonce, SharedProcessRawConnection.response);
			});
			this._register(toDisposable(() => { channelUnlisten(); rawUnlisten(); }));
		} else {
	...
	private async onWindowConnection(e: IpcMainEvent | any, nonce: string, responseChannel: string): Promise<void> {
		...
		// Tauri mode: shared process connections are not yet wired - return early
		if (isTauriMode()) {
			this.logService.trace('[SharedProcess] Tauri mode: shared process connections are not yet wired - returning early');
			return;
		}
