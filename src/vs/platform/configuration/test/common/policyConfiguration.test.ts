...
describe('PolicyConfiguration', () => {
	...
	beforeAll(() => Registry.as<IConfigurationRegistry>(Extensions.Configuration).registerConfiguration(policyConfigurationNode));
	afterAll(() => Registry.as<IConfigurationRegistry>(Extensions.Configuration).deregisterConfigurations([policyConfigurationNode]));
	...
	it('initialize: with policies', async () => {
	...
	it('initialize: no policies', async () => {
		await testObject.initialize();
		const acutal = testObject.configurationModel;
	...
	it('initialize: with policies but not registered', async () => {
	...
	it('initialize: with object type policy', async () => {
		const expected = {
			'microsoft': true,
			'github': 'stable',
			'other': 1,
	...
	it('initialize: with array type policy', async () => {
	...
	it('initialize: with boolean type policy as false', async () => {
	...
	it('initialize: with boolean type policy as true', async () => {
	...
	it('initialize: with object type policy ignores policy if value is not valid', async () => {
	...
	it('initialize: with object type policy ignores policy if there are duplicate keys', async () => {
	...
	it('change: when policy is added', async () => {
		await fileService.writeFile(policyFile, VSBuffer.fromString(JSON.stringify({ 'PolicySettingA': 'policyValueA' })));
		await testObject.initialize();
	...
	it('change: when policy is updated', async () => {
		await fileService.writeFile(policyFile, VSBuffer.fromString(JSON.stringify({ 'PolicySettingA': 'policyValueA' })));
		await testObject.initialize();
	...
	it('change: when policy is removed', async () => {
		await fileService.writeFile(policyFile, VSBuffer.fromString(JSON.stringify({ 'PolicySettingA': 'policyValueA' })));
		await testObject.initialize();
	...
	it('change: when policy setting is registered', async () => {
	...
	it('change: when policy setting is deregistered', async () => {
		await fileService.writeFile(policyFile, VSBuffer.fromString(JSON.stringify({ 'PolicySettingA': 'policyValueA' })));
		await testObject.initialize();
	...
	it('initialize: with internal policies', async () => {
