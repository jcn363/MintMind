...
describe('Localize Manifest', () => {
	...
	it('replaces template strings', function () {
	...
	it('replaces template strings with fallback if not found in translations', function () {
		const localizedManifest = localizeManifest(
			store.add(new NullLogger()),
			deepClone(manifest),
			{},
	...
	it('replaces template strings - command title & categories become ILocalizedString', function () {
		const localizedManifest = localizeManifest(
			store.add(new NullLogger()),
			deepClone(manifest),
			{
	...
	it('replaces template strings - is best effort #164630', function () {
