...
describe('QuickInput', () => { // https://github.com/microsoft/vscode/issues/147543
	...
	it('pick - activeItem is honored', async () => {
		const item = { label: 'foo' };
	...
	it('onDidChangeValue - gets triggered when .value is set', async () => {
	...
	it('keepScrollPosition - works with activeItems', async () => {
	...
	it('keepScrollPosition - works with items', async () => {
		const quickpick = store.add(controller.createQuickPick() as QuickPick<IQuickPickItem>);
	...
	it('selectedItems - verify previous selectedItems does not hang over to next set of items', async () => {
		const quickpick = store.add(controller.createQuickPick());
	...
	it('activeItems - verify onDidChangeActive is triggered after setting items', async () => {
		const quickpick = store.add(controller.createQuickPick());
	...
	it('activeItems - verify setting itemActivation to None still triggers onDidChangeActive after selection #207832', async () => {
		const quickpick = store.add(controller.createQuickPick());
		const item = { label: 'step 1' };
		quickpick.items = [item];
		quickpick.show();
