...
import { IProcessEnvironment, OS, isWindows } from '../../../base/common/platform.js';
...
export async function getResolvedShellEnv(configurationService: IConfigurationService, logService: ILogService, args: NativeParsedArgs, env: IProcessEnvironment): Promise<typeof process.env> {
  ...
  // Skip if running from CLI already
    ...
    logService.trace('resolveShellEnv(): skipped (MINTMIND_CLI is set)');
...
async function doResolveUnixShellEnv(logService: ILogService, token: CancellationToken): Promise<typeof process.env> {
  const runAsNode = process.env['TAURI_RUN_AS_NODE'];
  logService.trace('getUnixShellEnvironment#runAsNode', runAsNode);

  const noAttach = process.env['TAURI_NO_ATTACH_CONSOLE'];
  ...
  const env = {
    ...
    MINTMIND_RESOLVING_ENVIRONMENT: '1'
  };
  ...
  return new Promise<typeof process.env>((resolve, reject) => {
    ...
    child.on('close', (code, signal) => {
      ...
      try {
        ...
        if (runAsNode) {
          env['TAURI_RUN_AS_NODE'] = runAsNode;
        } else {
          delete env['TAURI_RUN_AS_NODE'];
        }

        if (noAttach) {
          env['TAURI_NO_ATTACH_CONSOLE'] = noAttach;
        } else {
          delete env['TAURI_NO_ATTACH_CONSOLE'];
        }

        delete env['MINTMIND_RESOLVING_ENVIRONMENT'];
