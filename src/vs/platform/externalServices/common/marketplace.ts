...
export async function resolveMarketplaceHeaders(version: string,
	...
	const headers: IHeaders = {
		'X-Market-Client-Id': `MintMind ${version}`,
		'User-Agent': `MintMindnd ${version} (${productService.nameShort})`
	};
	...
	if (supportsTelemetry(productService, environmentService) && getTelemetryLevel(configurationService) === TelemetryLevel.USAGE) {
		...
		// Send machineId as MintMindnd-SessionId so we can correlate telemetry events across different services
		// machineId can be undefined sometimes (eg: when launching from CLI), so send serviceMachineId instead otherwise
		// Marketplace will reject the request if there is no MintMindnd-SessionId header
		headers['MintMindnd-SessionId'] = telemetryService.machineId || serviceMachineId;
	}
