...
export class DiskFileSystemProviderClient extends Disposable implements
	...
	async stat(resource: URI): Promise<IStat> {
		...
		try {
			return await this.channel.call('stat', [resource]);
		} finally {
			this.logService.info(`File operation 'stat' took ${Date.now() - start}ms`);
		}
	}
	...
	async realpath(resource: URI): Promise<string> {
		const start = Date.now();
		try {
			return await this.channel.call('realpath', [resource]);
		} finally {
			this.logService.info(`File operation 'realpath' took ${Date.now() - start}ms`);
		}
	}
	...
	async readdir(resource: URI): Promise<[string, FileType][]> {
		const start = Date.now();
		try {
			return await this.channel.call('readdir', [resource]);
		} finally {
			this.logService.info(`File operation 'readdir' took ${Date.now() - start}ms`);
		}
	}
	...
	async readFile(resource: URI, opts?: IFileAtomicReadOptions): Promise<Uint8Array> {
		...
		try {
			...
			this.logService.info(`File operation 'readFile' took ${Date.now() - start}ms`);
		}
	}
	...
	async writeFile(resource: URI, content: Uint8Array, opts: IFileWriteOptions): Promise<void> {
		const start = Date.now();
		try {
			return await this.channel.call('writeFile', [resource, VSBuffer.wrap(content), opts]);
		} finally {
			this.logService.info(`File operation 'writeFile' took ${Date.now() - start}ms`);
		}
	}
	...
	async mkdir(resource: URI): Promise<void> {
		const start = Date.now();
		try {
			return await this.channel.call('mkdir', [resource]);
		} finally {
			this.logService.info(`File operation 'mkdir' took ${Date.now() - start}ms`);
		}
	}
	...
	async delete(resource: URI, opts: IFileDeleteOptions): Promise<void> {
		const start = Date.now();
		try {
			return await this.channel.call('delete', [resource, opts]);
		} finally {
			this.logService.info(`File operation 'delete' took ${Date.now() - start}ms`);
		}
	}

	async rename(resource: URI, target: URI, opts: IFileOverwriteOptions): Promise<void> {
		const start = Date.now();
		try {
			return await this.channel.call('rename', [resource, target, opts]);
		} finally {
			this.logService.info(`File operation 'rename' took ${Date.now() - start}ms`);
		}
	}

	async copy(resource: URI, target: URI, opts: IFileOverwriteOptions): Promise<void> {
		const start = Date.now();
		try {
			return await this.channel.call('copy', [resource, target, opts]);
		} finally {
			this.logService.info(`File operation 'copy' took ${Date.now() - start}ms`);
		}
	}
	...
	async cloneFile(resource: URI, target: URI): Promise<void> {
		const start = Date.now();
		try {
			return await this.channel.call('cloneFile', [resource, target]);
		} finally {
			this.logService.info(`File operation 'cloneFile' took ${Date.now() - start}ms`);
		}
	}
