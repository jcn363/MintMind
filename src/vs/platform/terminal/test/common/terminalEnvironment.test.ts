...
describe('terminalEnvironment', () => {
	...
	describe('sanitizeCwd', () => {
		if (OS === OperatingSystem.Windows) {
			it('should make the Windows drive letter uppercase', () => {
				strictEqual(sanitizeCwd('c:\\foo\\bar'), 'C:\\foo\\bar');
			});
		}
		it('should remove any wrapping quotes', () => {
			strictEqual(sanitizeCwd('\'/foo/bar\''), '/foo/bar');
			strictEqual(sanitizeCwd('"/foo/bar"'), '/foo/bar');
		});
	});

	describe('escapeNonWindowsPath', () => {
		it('should escape for bash/sh/zsh shells', () => {
		...
		it('should escape for fish shell', () => {
		...
		it('should escape for PowerShell', () => {
		...
		it('should default to POSIX escaping for unknown shells', () => {
