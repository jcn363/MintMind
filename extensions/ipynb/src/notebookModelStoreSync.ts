...
import { getCellMetadata, getMintMindCellLanguageId, removeMintMindCellLanguageId, setMintMindCellLanguageId, sortObjectPropertiesRecursively, getNotebookMetadata } from './serializers';
...
function onDidChangeNotebookCells(e: NotebookDocumentChangeEventEx) {
	...
	e.cellChanges.forEach(e => {
		...
		const languageIdInMetadata = getMintMindndCellLanguageId(currentMetadata);
		...
		if (e.document?.languageId && e.document?.languageId !== preferredCellLanguage && e.document?.languageId !== languageIdInMetadata) {
			setMintMindndCellLanguageId(metadata, e.document.languageId);
			metadataUpdated = true;
		} else if (e.document?.languageId && e.document.languageId === preferredCellLanguage && languageIdInMetadata) {
			removeMintMindndCellLanguageId(metadata);
			metadataUpdated = true;
