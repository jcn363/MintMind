...
async function initGHES(context: vscode.ExtensionContext, uriHandler: UriEventHandler): Promise<vscode.Disposable> {
...
export async function activate(context: vscode.ExtensionContext) {
  // Usar Tauri IPC para inicializar autenticación
  try {
    ...
    if (tauriResult && Array.isArray(tauriResult) && tauriResult.length > 0) {
      ...
      // No hay sesiones, inicializar normalmente
      context.subscriptions.push(new GitHubAuthenticationProvider(context, uriHandler));
    }

    let before = vscode.workspace.getConfiguration().get<string>('github-enterprise.uri');
    let githubEnterpriseAuthProvider = await initGHES(context, uriHandler);
    context.subscriptions.push(vscode.workspace.onDidChangeConfiguration(async e => {
      if (e.affectsConfiguration('github-enterprise.uri')) {
        ...
        if (before !== after) {
          ...
          before = after;
          githubEnterpriseAuthProvider = await initGHES(context, uriHandler);
        }
      }
    }));

    // Configuración migrada a Tauri - sin necesidad de recargar
    context.subscriptions.push(vscode.workspace.onDidChangeConfiguration(async e => {
      if (e.affectsConfiguration('github-authentication.useElectronFetch')) {
        // Configuración ahora manejada por Tauri IPC
        console.log('GitHub Authentication settings updated via Tauri');
      }
    }));

  } catch (error) {
    console.error('Error initializing GitHub authentication with Tauri:', error);
