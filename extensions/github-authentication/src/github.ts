...
export class GitHubAuthenticationProvider implements vscode.AuthenticationProvider, vscode.Disposable {
	...
	public async createSession(scopes: string[], options?: GitHubAuthenticationProviderOptions): Promise<vscode.AuthenticationSession> {
	  try {
	    ...
	    // Usar Tauri IPC para login
	    const loginResult = await invoke('github_login', {
	      clientId: 'your_github_client_id', // Configurar con client ID real
	      scopes: sortedScopes,
	      tenant: 'common', // Para GitHub no es relevante, pero mantenemos compatibilidad
	    });

	    if (loginResult && loginResult.success) {
	      const token = loginResult.token.access_token;
	      ...
	      this._logger.info('Login success via Tauri!');

	      return session;
	    } else {
	      throw new Error(loginResult?.error || 'Login failed via Tauri');
	    }
	  } catch (e) {
	    // Fallback a implementación original si Tauri falla
	    try {
	      const sessions = await this._sessionsPromise;
	      const loginWith = options?.account?.label;
	      const signInProvider = options?.provider;
	      this._logger.info(`Logging in with${signInProvider ? ` ${signInProvider}, ` : ''} '${loginWith ? loginWith : 'any'}' account (fallback)...`);
	      ...
	      this._sessionChangeEmitter.fire({ added: [session], removed, changed: [] });
	      this._logger.info('Login success (fallback)!');
