...
function withNodeDefaults(/**@type WebpackConfig & { context: string }*/extConfig) {
	const defaultConfig = {
		...
		// Enhanced optimization for tree shaking and lazy loading
		optimization: {
			usedExports: true,
			sideEffects: false,
			splitChunks: {
	...
	// Apply lazy loading configuration to the final config
	if (extConfig.optimization) {
		if (extConfig.optimization.splitChunks) {
			// Merge with existing split chunks configuration
			const mergedChunks = merge(lazyLoadingConfig.optimization.splitChunks, extConfig.optimization.splitChunks);
			extConfig.optimization.splitChunks = mergedChunks;
		} else {
			// Add lazy loading split chunks
			extConfig.optimization.splitChunks = lazyLoadingConfig.optimization.splitChunks;
		}
	} else {
		// Add the entire optimization config
		extConfig.optimization = lazyLoadingConfig.optimization;
	}
...
function withBrowserDefaults(/**@type WebpackConfig & { context: string }*/extConfig, /** @type AdditionalBrowserConfig */ additionalOptions = {}) {
	...
	const defaultConfig = {
		...
		// Enhanced browser optimization with lazy loading
		optimization: {
			usedExports: true,
			sideEffects: false,
			splitChunks: lazyLoadingConfig.optimization.splitChunks,
		},
		module: {
			rules: [{
				test: /\.ts$/,
...
export { withNodeDefaults as node, withBrowserDefaults as browser, nodePlugins, browserPlugins, lazyLoadingConfig };
