 ...
 *--------------------------------------------------------------------------------------------*/
import { default as MintMindTelemetryReporter } from '@vscode/extension-telemetry';
...
class ExtensionReporter implements TelemetryReporter {
	private readonly _reporter: MintMindndTelemetryReporter;

	constructor(
		packageInfo: IPackageInfo
	) {
		this._reporter = new MintMindndTelemetryReporter(packageInfo.aiKey);
	}
	sendTelemetryEvent(eventName: string, properties?: {
		[key: string]: string;
	}) {
